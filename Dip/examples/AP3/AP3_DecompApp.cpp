...
// --------------------------------------------------------------------- //
void AP3_DecompApp::initializeApp(UtilParameters &utilParam) throw(CoinError) {
  ...
  string fileName =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.Instance + ".txt";
  m_ap3data.readInstance(fileName.c_str());
  m_ap3data.m_instance = m_appParam.Instance;

  string fileNameSol =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.Instance + ".sol";
  ...
  // TODO: don't bother building if not using LP solver for this
  {
    ...
    CoinPackedMatrix M(false, dimensionSq, n_rows, n_nonzeros, rowEls, rowInd,
                       rowBeg, rowLen);
...
// --------------------------------------------------------------------- //
void AP3_DecompApp::APPcreateModel(
...
//--------------------------------------------------------------------- //
void AP3_DecompApp::createModelPart(
    const int modelType, int *rowInd, double *rowEls,
    DecompConstraintSet *modelCore,
    DecompConstraintSet *modelRelax) throw(CoinError) {

  UtilPrintFuncBegin(m_osLog, m_classTag, "createModelPart()",
                     m_param.LogDebugLevel, 2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "createModelPart()",
                   m_param.LogDebugLevel, 2);
}
...
//--------------------------------------------------------------------- //
DecompStatus AP3_DecompApp::APPsolveRelaxed(
    ...
    const bool checkDup, OsiSolverInterface *m_subprobSI,
    list<DecompVar *> &vars) {
  ...
  // doPush = 0;
  if (doPush) {
    UTIL_DEBUG(m_param.LogDebugLevel, 5, var->print(););
    vars.push_back(var);
  } else
    UTIL_DELPTR(var);
...
//--------------------------------------------------------------------- //
void AP3_DecompApp::printOriginalColumn(const int index, ostream *os) const {
  int i, j, k;
