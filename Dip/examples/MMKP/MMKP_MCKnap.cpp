...
void MMKP_MCKnap::solveTrivialMaxSum(const double *redCost,
                                     ...
                                     vector<int> &solInd, double &varRedCost,
                                     double &varOrigCost) {
  ...
  for (i = 0; i < m_nGroupRows; i++) {
    ...
    printf("i:%d j:%d redCost:%g minRedCost:%g wt:%d minWeight:%d\n", i, 0,
           redCost[ijIndex], minRedCost, m_weight[ijIndex], minWeight);
#endif
...
// --------------------------------------------------------------------- //
void MMKP_MCKnap::setMCKnapData(const double capacity, const double *weight) {
  ...
  //---
  m_wscale = UtilScaleDblToIntArr(m_nCols, weight, m_weight, capacity,
                                  &m_capacity, MCKP_EPSILON);
...
// --------------------------------------------------------------------- //
void MMKP_MCKnap::solveMCKnap(const double *redCost, const double *origCost,
                              vector<int> &solInd, vector<double> &solEls,
                              double &varRedCost, double &varOrigCost) {
  ...
  for (i = 0; i < m_nCols; i++) {
    ...
    printf("\ncostDbl[%d: %d, %d]: %g", i, ij.first, ij.second, m_costDbl[i]);
  }
#endif
  ...
  for (i = 0; i < m_nCols; i++) {
    ...
    printf("\n[%d: %d, %d]: dbl-> %12.5f int-> %8d diff-> %12.5f", i, ij.first,
           ij.second, m_costDbl[i], m_cost[i], diff);
    assert(diff < 0.99);
  }
#endif
  ...
  for (i = 0; i < m_nCols; i++) {
    if (m_cost[i] < 0) {
      throw UtilException("negative cost value", "solveMCKnap", "MMKP_MCKnap");
    }
  }
  ...
  for (i = 0; i < m_setset->size; i++) {
    ...
    for (j = 0; j < setPtr->size; j++) {
      ...
      recPtr->wsum = m_weight[colIndex];
#ifdef MMKP_MCKNAP_DEBUG
      printf("\ncolIndex: %d i: %d, j: %d, p: %d, w: %d", colIndex, i, j,
             recPtr->psum, recPtr->wsum);
#endif
      recPtr++;
      colIndex++;
    }
...
#ifdef MMKP_MCKNAP_DEBUG
      printf("\nc: %d = (%d,%d) redCost: %g cost: %d wt: %d", c, i, j,
  ...
  case MCKNAP_RC_TRIVIAL_MAXSUM:
    ...
    solveTrivialMaxSum(redCost, origCost, solInd, varRedCost, varOrigCost);
#ifdef MMKP_MCKNAP_DEBUG
