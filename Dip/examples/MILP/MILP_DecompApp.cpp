...
void MILP_DecompApp::createModels() {
  ...
  UTIL_MSG(m_appParam.LogLevel, 3, (*m_osLog) << "Core  Rows:";
           ...
           (*m_osLog) << "\nRelax Rows:";
           for (i = 0; i < nRowsRelax; i++)(*m_osLog) << rowsRelax[i] << " ";
           (*m_osLog) << "\n";);
  ...
  m_modelRandCore.M->submatrixOf(*m_mpsIO.getMatrixByRow(), nRowsCore,
                                 rowsCore);
  ...
  m_modelRandRelax.M->submatrixOf(*m_mpsIO.getMatrixByRow(), nRowsRelax,
                                  rowsRelax);
