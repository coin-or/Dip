...
int main(int argc, char **argv) {
  try {
    ...
    cout << "Status= " << status << " BestLB=  " << setw(10)
         << UtilDblToStr(alpsModel.getGlobalLB(), 5) << " BestUB= " << setw(10)
    ...
    if (status == AlpsExitStatusOptimal && tsp.getBestKnownUB() < 1.0e50) {
      ...
      //---
      double diff = fabs(tsp.getBestKnownUB() - alpsModel.getGlobalUB());
      if (diff > 1.0e-4) {
        cerr << "ERROR. BestKnownUB= " << tsp.getBestKnownUB()
             << " but DECOMP claims GlobalUB= " << alpsModel.getGlobalUB()
             << endl;
