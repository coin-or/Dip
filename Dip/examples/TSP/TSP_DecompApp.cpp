...
void TSP_DecompApp::initializeApp() {
  ...
  //---
  string fileName =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.Instance + ".tsp";
  ...
  //---
  m_tsp.m_boost.buildCompleteGraphMinusVert(m_tsp.m_vert, graphLib.n_vertices);
...
void TSP_DecompApp::createModel2Match(DecompConstraintSet *modelCS) {
  ...
  //---
  UtilPrintFuncBegin(m_osLog, m_classTag, "createModel2Match()",
                     m_appParam.LogLevel, 2);
  ...
  modelCS->M =
      new CoinPackedMatrix(true, rowIndices, colIndices, elements, 2 * n_cols);

  UTIL_DELARR(rowIndices);
  UTIL_DELARR(colIndices);
  UTIL_DELARR(elements);

  UtilPrintFuncEnd(m_osLog, m_classTag, "createModel2Match()",
                   m_appParam.LogLevel, 2);
}
...
void TSP_DecompApp::createModelTrivialSEC(DecompConstraintSet *modelCS) {
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "createModelTrivialSEC()",
                     m_appParam.LogLevel, 2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "createModelTrivialSEC()",
                   m_appParam.LogLevel, 2);
}

//===========================================================================//
int TSP_DecompApp::generateCuts(const double *x, DecompCutList &newCuts) {

  UtilPrintFuncBegin(m_osLog, m_classTag, "generateCuts()", m_appParam.LogLevel,
                     2);

  int n_cuts = 0;
  UtilGraphLib &graphLib = m_tsp.m_graphLib;
  TSP_Concorde &tspConcorde = m_tsp.m_concorde;
  tspConcorde.buildSubGraph(graphLib.n_vertices, graphLib.n_edges, x);

  if (m_appParam.CutSubtoursX)
    n_cuts += generateCutsSubtour(newCuts);

  UtilPrintFuncEnd(m_osLog, m_classTag, "generateCuts()", m_appParam.LogLevel,
                   2);
  return n_cuts;
}

//===========================================================================//
DecompSolverStatus TSP_DecompApp::solveRelaxed(const int whichBlock,
                                               const double *redCostX,
                                               const double convexDual,
                                               DecompVarList &varList) {

  UtilPrintFuncBegin(m_osLog, m_classTag, "solveRelaxed()", m_appParam.LogLevel,
                     2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "solveRelaxed()", m_appParam.LogLevel,
                   2);
  return solverStatus;
}

//===========================================================================//
void TSP_DecompApp::solveOneTree(const double *cost, const double alpha,
                                 vector<pair<int, double>> &edge_cost,
                                 DecompVarList &vars, Graph &g) {

  UtilPrintFuncBegin(m_osLog, m_classTag, "solveOneTree()", m_appParam.LogLevel,
                     2);
  ...
  //---
  int sort_size =
      std::min<int>(static_cast<int>(edge_cost.size()), 2 + max_exchanges);
  partial_sort(edge_cost.begin(), edge_cost.begin() + sort_size,
  ...
  for (tie(ei, ei_end) = edges(g); ei != ei_end; ei++) {
    ...
    if (m_appParam.LogLevel > 4)
      (*m_osLog) << "cost edge( " << source(*ei, g) << "," << target(*ei, g)
                 << "): " << cost[index];
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "solveOneTree()", m_appParam.LogLevel,
                   2);
}

//===========================================================================//
bool TSP_DecompApp::APPisUserFeasible(const double *x, const int n_cols,
...
//===========================================================================//
void TSP_DecompApp::printOriginalColumn(const int index, ostream *os) const {
