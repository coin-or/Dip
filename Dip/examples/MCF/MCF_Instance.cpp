...
//===========================================================================//
int MCF_Instance::readInstance(string &fileName, bool addDummyArcs) {
  ifstream is;
  ...
  while (is.good()) {
    ...
    switch (line[0]) {
    case 'p':
      if (sscanf(line, "p%s%i%i%i", name, &m_numNodes, &m_numArcs,
                 &m_numCommodities) != 4) {
      ...
      m_problemName = name;
      m_arcs = new arc[m_numArcs + (addDummyArcs ? m_numCommodities : 0)];
    ...
    case 'd':
      if (sscanf(line, "d%i%i%i", &m_commodities[commodities_read].source,
    ...
    case 'a':
      if (sscanf(line, "a%i%i%i%i%lf", &m_arcs[arcs_read].tail,
                 &m_arcs[arcs_read].head, &m_arcs[arcs_read].lb,
                 &m_arcs[arcs_read].ub, &m_arcs[arcs_read].weight) != 5) {
  ...
  if (!size_read || arcs_read != m_numArcs ||
