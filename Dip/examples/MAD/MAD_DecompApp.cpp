...
// --------------------------------------------------------------------- //
int MAD_DecompApp::isOrtho(const int *rowInd1, const int *rowInd2,
                           const int rowLen1, const int rowLen2) {
...
void MAD_DecompApp::initializeApp(UtilParameters &utilParam) {
  ...
  if (m_appParam.Capacity != -1) {
    ...
    m_kappa = static_cast<int>(ceil(static_cast<double>(m_nOrigRows) / m_beta));
...
#if 1
// --------------------------------------------------------------------- //
void MAD_DecompApp::APPcreateModel(
    double *&objCoeff, map<int, DecompConstraintSet *> &modelCore,
  ...
  //---
  UtilPrintFuncBegin(m_osLog, m_classTag, "APPcreateModel()",
                     m_param.LogDebugLevel, 2);
...
#endif
  UTIL_DEBUG(m_param.LogDebugLevel, 4, m_cliquer->printGraph(m_cliquer->m_g););
  ...
  for (i = 0; i < m_nOrigRows; i++) {
    ...
    cModel.addRow(row.getNumElements(), row.getIndices(), row.getElements());
    ...
    for (b1 = 0; b1 < m_beta; b1++) {
      for (b2 = 0; b2 < m_beta; b2++) {
        if (b1 == b2)
          continue;
        m_cliquer->delEdge(m_cliquer->m_g, xIndex(i, b1), xIndex(i, b2));
      }
    }
#endif
#ifdef __MAD_USE_QUALEX__
  ...
  for (i = 0; i < m_nOrigRows; i++) {
    for (j = (i + 1); j < m_nOrigRows; j++) {
      ...
      if (!isOrtho(rowI.getIndices(), rowJ.getIndices(), rowI.getNumElements(),
                   rowJ.getNumElements())) {
        //---
        ...
        for (b1 = 0; b1 < m_beta; b1++) {
          for (b2 = 0; b2 < m_beta; b2++) {
            ...
            cModel.addRow(row.getNumElements(), row.getIndices(),
                          row.getElements());

#ifdef __MAD_USE_CLIQUER__
            m_cliquer->delEdge(m_cliquer->m_g, xIndex(i, b1), xIndex(j, b2));
            m_cliquer->delEdge(m_cliquer->m_g, xIndex(j, b1), xIndex(i, b2));
#endif
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "APPcreateModel()",
                   m_param.LogDebugLevel, 2);
}
#else
...
void MAD_DecompApp::APPcreateModel(
  ...
  CoinAssertHint(modelCoreCl->M, "Error: Out of Memory");
  modelCoreCl->M->setDimensions(0, n_cols);
  modelCoreCl->M->reserve(m_nOrigRows + m_beta, 2 * m_beta * m_nOrigRows);
...
DecompStatus MAD_DecompApp::APPsolveRelaxed(
    const int whichModel, const double *redCostX, const double *origCost,
    const double alpha, const int n_origCols, const bool checkRC,
    const bool checkDup, bool &isExact, OsiSolverInterface *m_subprobSI,
    list<DecompVar *> &vars) {
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "APPsolveRelaxed()",
                     m_param.LogDebugLevel, 2);
  ...
  nGreedyPts =
      heuristicGreedy(greedyPoints, INCREASING, redCostX, origCost, redCostX);
  for (i = 0; i < nGreedyPts; i++) {
    ...
    UTIL_DEBUG(
        ...
        (*m_osLog) << "Greedy DecompVar origCost = " << gp.solValueOrigCost
    ...
    if (!checkRC || ((gp.solValueRedCost + alpha) < -DecompEpsilon)) {
      vars.push_back(new DecompVar(n_origCols, gp.solution,
  ...
  UTIL_DEBUG(m_param.LogDebugLevel, 4, printf("\nGRAPH m_g:\n");
             m_cliquer->printGraph(m_cliquer->m_g););
  ...
  m_cliquer->copyGraphNonPos(m_gStar, m_cliquer->m_g, redCostX);
  printf("timer after copy graph = %12.10f\n", thisTimer.timeElapsed());

  UTIL_DEBUG(
      m_param.LogDebugLevel, 4, printf("\nGRAPH m_gStar:\n");
  ...
  double offset = 0.0;
  double minrc =
      *min_element(redCostXNegOffset, redCostXNegOffset + n_origCols);
  if (minrc <= 0) {
    offset = -minrc + 1;
  ...
  UTIL_DEBUG(
      m_param.LogDebugLevel, 4, for (i = 0; i < m_nOrigRows; i++) {
        for (b = 0; b < m_beta; b++) {
          cout << "x[ " << i << "," << b << " -> " << xIndex(i, b)
  ...
  for (it = m_qualex->m_clique.begin(); it != m_qualex->m_clique.end(); it++) {
    varRedCost += redCostX[*it];
    UTIL_DEBUG(m_param.LogDebugLevel, 4,
               printf("\nGreedy ind: %d redCostX: %g varRedCost: %g", *it,
                      redCostX[*it], varRedCost););
  }
  UTIL_DEBUG(m_param.LogDebugLevel, 4,
  ...
  if (!checkRC || ((varRedCost + alpha) < -DecompEpsilon)) {
    ...
    varRedCost = 0.0;
    varOrigCost = 0.0;
    vector<int> apInd(m_qualex->m_clique.begin(), m_qualex->m_clique.end());
    vector<double> apEls(m_qualex->m_clique.size(), 1.0);

    for (it = m_qualex->m_clique.begin(); it != m_qualex->m_clique.end();
         it++) {
      varRedCost += redCostX[*it];
      varOrigCost += origCost[*it];

      UTIL_DEBUG(m_param.LogDebugLevel, 4,
                 printf("\nind: %d redCostX: %g varRedCost: %g", *it,
                        redCostX[*it], varRedCost););
    }
    UTIL_DEBUG(m_param.LogDebugLevel, 4,
               printf("\nvarRC = %g, alpha = %g", varRedCost, alpha););
    if (!checkRC || ((varRedCost + alpha) < -DecompEpsilon)) {
      ...
      DecompVar *var =
          new DecompVar(apInd, apEls, varRedCost + alpha, varOrigCost);
      vars.push_back(var);
  ...
  //---
  int scaleFactor =
      UtilScaleDblToIntArr(n_origCols, redCostX, redCostXInt, alpha, &alphaInt);
  ...
  UTIL_DEBUG(m_param.LogDebugLevel, 4, m_cliquer->cliquePrintAll(m_gStar););
...
int MAD_DecompApp::generateInitVars(DecompVarList &initVars) {
  ...
  for (i = 0; i < nGreedyPts; i++) {
    ...
    //---
    UTIL_DEBUG(m_param.LogDebugLevel, 4,
               (*m_osLog) << "Greedy DecompVar origCost = "
                          << gp.solValueOrigCost;
               printOriginalSolution(nOrigCols, gp.solution, &cout););
    // TODO: might be more efficient if return sparse... greedySol
    initVars.push_back(
        new DecompVar(nOrigCols, gp.solution, 0, gp.solValueOrigCost));
  }
...
//--------------------------------------------------------------------- //
void MAD_DecompApp::printOriginalColumn(const int index, ostream *os) const {
