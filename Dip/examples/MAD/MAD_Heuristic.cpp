...
// --------------------------------------------------------------------- //
// name?
int MAD_DecompApp::APPheuristics(const double *xhat, const double *origCost,
...
int MAD_DecompApp::heuristicGreedy(vector<GreedyPoint> &greedyPoints,
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "heuristicGreedy()",
  ...
  UTIL_DELARR(blocks);
  UtilPrintFuncEnd(m_osLog, m_classTag, "heuristicGreedy()",
                   m_param.LogDebugLevel, 2);
...
int MAD_DecompApp::heuristicGreedy(const MAD_HeurSortDir sortDir,
  ...
  for (i = 0; i < nGreedyPts; i++) {
    ...
    UTIL_DEBUG(m_param.LogDebugLevel, 4,
               (*m_osLog) << "Greedy DecompSolution cost = "
                          << gp.solValueOrigCost << endl;
               printOriginalSolution(n_origCols, gp.solution, &cout););
    // TODO: for use in app solved - might not want this...
    solVec.push_back(new MAD_DecompSolution(this, n_origCols, gp.solution,
                                            gp.solValueOrigCost));
  }
  return nGreedyPts;
}
