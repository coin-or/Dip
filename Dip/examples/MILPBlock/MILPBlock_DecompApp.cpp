...
void MILPBlock_DecompApp::readBlockFile() {
  ...
  ifstream is;
  string fileName = m_appParam.DataDir + UtilDirSlash() + m_appParam.BlockFile;
  ...
  string fileNameP =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.PermuteFile;
  ...
  if (m_appParam.BlockFileFormat == "List" ||
    ...
    cerr << "Error: BlockFileFormat = " << m_appParam.BlockFileFormat
         << " is an invalid type. Valid types = (List,Pair,PairName)." << endl;
    throw UtilException("Invalid Parameter.", "readBlockFile",
                        "MILPBlock_DecompApp");
  }
...
void MILPBlock_DecompApp::readInitSolutionFile(DecompVarList &initVars) {
  ...
  ifstream is;
  string fileName =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.InitSolutionFile;
...
//===========================================================================//
void MILPBlock_DecompApp::findActiveColumns(const vector<int> &rowsPart,
...
*/
//===========================================================================//
void MILPBlock_DecompApp::createModelPart(DecompConstraintSet *model,
                                          const int nRowsPart,
                                          const int *rowsPart) {
...
//===========================================================================//
void MILPBlock_DecompApp::createModelPartSparse(DecompConstraintSet *model,
                                                const int nRowsPart,
                                                const int *rowsPart) {
...
void MILPBlock_DecompApp::createModels() {
  ...
  UTIL_MSG(m_appParam.LogLevel, 3, (*m_osLog) << "Core  Rows:";
  ...
  if (!m_objective)
    throw UtilExceptionMemory("createModels", "MILPBlock_DecompApp");
  memcpy(m_objective, m_mpsIO.getObjCoefficients(), nCols * sizeof(double));
