...
void GAP_DecompApp::initializeApp() {
  ...
  for (k = 0; k < nMachines; k++) {
    knapK = new GAP_KnapPisinger(nTasks, capacity[k], weight + (k * nTasks),
                                 profit + (k * nTasks));
...
int GAP_DecompApp::createModelPartAP(DecompConstraintSet *model) {
  ...
  int nRows = nTasks;
  UtilPrintFuncBegin(m_osLog, m_classTag, "createModelPartAP()",
                     m_appParam.LogLevel, 2);
  ...
  for (i = 0; i < nMachines; i++) {
    for (j = 0; j < nTasks; j++) {
      string colName = "x(" + UtilIntToStr(colIndex) + "_" + UtilIntToStr(i) +
                       "," + UtilIntToStr(j) + ")";
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "createModelPartAP()",
                   m_appParam.LogLevel, 2);
  return status;
}
...
DecompSolverStatus GAP_DecompApp::solveRelaxed(const int whichBlock,
  ...
  m_knap[whichBlock]->solve(whichBlock, redCostXB, origCostB, solInd, solEls,
  ...
  DecompVar *var = new DecompVar(solInd, solEls, varRedCost, varOrigCost);
...
//--------------------------------------------------------------------- //
void GAP_DecompApp::printOriginalColumn(const int index, ostream *os) const {
