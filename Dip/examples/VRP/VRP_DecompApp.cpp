...
void VRP_DecompApp::initializeApp() {
  ...
  //---
  string fileName =
      m_appParam.DataDir + UtilDirSlash() + m_appParam.Instance + ".vrp";
  ...
  //---    7. Upper Bound
  //---
  string bestKnownFile = m_appParam.DataDir + UtilDirSlash() + "vrplib.opt";
  ifstream is;
  string instanceName;
  int numCustomers;
  int numVehicles;
  ...
  while (!is.eof()) {
    is >> instanceName >> numCustomers >> numVehicles >> capacity >>
        tightness >> gap >> upperBound;
...
void VRP_DecompApp::createModelTwoDegree(DecompConstraintSet *model) {
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "createModelTwoDegree()",
                     m_appParam.LogLevel, 2);
  ...
  CoinAssertHint(rowIndices && colIndices && elements, "Error: Out of Memory");
  ...
  //---
  model->M =
      new CoinPackedMatrix(true, rowIndices, colIndices, elements, nEdgesX2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "createModelTwoDegree()",
                   m_appParam.LogLevel, 2);
}

//===========================================================================//
int VRP_DecompApp::generateCuts(const double *x, DecompCutList &newCuts) {
...
DecompSolverStatus VRP_DecompApp::solveRelaxed(const int whichBlock,
  ...
  if (m_appParam.ModelNameRelax == "MTSP") {
    ...
    DecompVar *var = new DecompVar(vrpRouteInd, vrpRouteEls,
...
//===========================================================================//
bool VRP_DecompApp::APPisUserFeasible(const double *x, const int n_cols,
  ...
  UTIL_DEBUG(
      m_appParam.LogLevel, 5, (*m_osLog) << "m_sg:\n";
      ...
      const DecompAlgo *decompAlgo = getDecompAlgo();
      string fileNameDot =
          baseName + ".n" + UtilIntToStr(decompAlgo->getNodeIndex()) + ".c" +
          UtilIntToStr(decompAlgo->getCutCallsTotal()) + ".p" +
          UtilIntToStr(decompAlgo->getPriceCallsTotal()) + ".dot";
      m_boost.printDotFile(fileNameDot, graphLib.vertex_wt, m_boost.m_sg););
  ...
  int c;
  int n_comp = m_boost.findConnectedComponents(m_boost.m_sg0, component);
  ...
  if (n_comp != (m_vrp.m_numRoutes + 1)) {
    UTIL_DEBUG(m_appParam.LogLevel, 2,
               (*m_osLog) << "not feasible, n_comp=" << n_comp
...
//===========================================================================//
void VRP_DecompApp::printOriginalColumn(const int index, ostream *os) const {
