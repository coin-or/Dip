...
class TestCuttingStockProblem(DippyTestCase):
    ...
    def test_dippy_relaxation(self):
        ...
        dippy.Solve(self.prob, {"doPriceCut": 1, "CutCGL": 0})
        self.assertAlmostEqual(self.prob.objective.value(), 2.0)
        self.variable_feasibility_test(self.prob)
        self.constraint_feasibility_test(self.prob)
...
def create_cutting_stock_problem(doRelaxed=False):
    ...
    CUTS = [(p, i) for p in PATTERNS for i in ITEMS]
    ...
    # Meet demand
    for i in ITEMS:
        prob += sum(cutVars[(p, i)] for p in PATTERNS) >= demand[i]
    ...
    for p in PATTERNS:
        if doRelaxed:
            prob.relaxation[p] += (
                sum(length[i] * cutVars[(p, i)] for i in ITEMS)
    ...
    def relaxed_solver(prob, patt, redCosts, convexDual):
        ...
        item_idx = [i for i in ITEMS if redCosts[cutVars[(patt, i)]] < 0]
        ...
        if rc < 0:  # Using this pattern
            var_values = dict(
                [(v, solution[i]) for i, v in enumerate(vars) if solution[i] > 0]
            )
            if totalCut > 0:
