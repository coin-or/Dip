...
class TestFacilityProblem(DippyTestCase):
    ...
    def test_dippy_cuts(self):
        ...
        dippy.Solve(self.prob, {"TolZero": "%s" % self.tol})
        self.assertAlmostEqual(self.prob.objective.value(), 212.0)
        self.variable_feasibility_test(self.prob)
        self.constraint_feasibility_test(self.prob)
...
def create_facility_problem():
    ...
    REQUIREMENT = {1: 66, 2: 4, 3: 85, 4: 93, 5: 68, 6: 76, 7: 74, 8: 39, 9: 66, 10: 17}
    # Set of all products
    ...
    assign_vars = LpVariable.dicts(
        "AtLocation", [(i, j) for i in LOCATIONS for j in PRODUCTS], 0, 1, LpBinary
    )
    ...
    def choose_antisymmetry_branch(prob, sol):
        ...
        if (up - num_locations > tol) and (
            ...
            up_branch_lb = dict(
                [(use_vars[LOCATIONS[n]], 1) for n in range(0, int(up))]
            )
            ...
            for i in LOCATIONS:
                for j in PRODUCTS:
                    if j not in S[i]:  # If this product is not in the subset
                        if (sol[assign_vars[(i, j)]] > bestValue) and (
                            REQUIREMENT[j] <= mu[i]
                        ):
