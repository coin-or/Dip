...
def create_coke_problem():
    ...
    CUST_TRANSPORT = read_table(CUST_TRANSPORT_DATA, int, transpose=True)
    ...
    # objective
    prob += (
        1e6 * sum(buildVars[(l, s)] * SIZE_COSTS[s] for (l, s) in LOCATIONS_SIZES)
        + sum(flowVars[(s, d)] * ARC_COSTS[(s, d)] for (s, d) in ARCS),
    ...
    for loc in LOCATIONS:
        prob += (
            sum(flowVars[(loc, c)] for c in CUSTOMERS)
            <= sum(buildVars[(loc, s)] * s for s in SIZES),
            "Plant_%s_availability" % loc,
        )

    # one size
    for loc in LOCATIONS:
        prob += sum(buildVars[(loc, s)] for s in SIZES) == 1, "Plant_%s_size" % loc

    # conserve flow (mines)
    # flows are in terms of tonnes of coke
    for m in MINES:
        prob += (
            sum(flowVars[(m, j)] for j in LOCATIONS) <= MINE_SUPPLY[m],
            "Supply_mine_%s" % m,
        )

    for loc in LOCATIONS:
        prob += (
            sum(flowVars[(m, loc)] for m in MINES)
            - CC * sum(flowVars[(loc, c)] for c in CUSTOMERS)
            == 0,
            "Conserve_flow_location_%s" % loc,
        )

    for c in CUSTOMERS:
        prob += (
            sum(flowVars[(loc, c)] for loc in LOCATIONS) >= CUSTOMER_DEMAND[c],
            "Demand_cust_%s" % c,
        )
