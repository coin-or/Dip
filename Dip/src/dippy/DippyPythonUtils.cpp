...
PyObject *pyTupleList_FromNode(DecompAlgo *algo, DecompStatus decompStatus) {
  ...
  /** MO. 29/2/2012 - This section was originally an attempt to add "simple"
  cuts. i.e., that the sum of non-basic variables >= 1 (or at least a variant
  for lb and ub), so I passed the variable bounds at the node and lists of
  basic, lower bound and upper bound variables from the original problem.
  However, the cuts added "clashed" with the CGL cuts, probably because the
  variables introduced by these cuts, e.g., slacks, were not considered. I have
  abandoned this direction for now, but I like the idea of getting the full node
  problem so more complex cuts that use, e.g., the basis, can be implemented in
  Python.
  ...
  for (int j=0; j<numOrig; j++) {
       ...
       addTupleToPyList(pBoundList, PyList_GetItem(app->m_colList, j),
  pBoundPair);
  }
  ...
  if (algo->getMasterOSI()->basisIsAvailable()) {
       ...
       // MO (28/2/2012) - Assuming that any extra variables are added at the
  end, is this true? for (int j=0; j<numOrig; j++) switch (cstat[j]) { case 1:
  // basic PyList_Append(pBasisList, PyList_GetItem(app->m_colList, j)); break;
               case 2: // upper
                       PyList_Append(pUBList, PyList_GetItem(app->m_colList,
  j)); break; case 3: // lower PyList_Append(pLBList,
  PyList_GetItem(app->m_colList, j)); break; default: throw UtilException("Error
  calling method pyTupleList_FromNode()", "pyTupleList_FromNode",
  "DippyPythonUtils");
               }
...
void pyColDict_AsPairedVector(PyObject *pColDict,
  ...
  for (int i = 0; i < len; i++) {
    ...
    if ((index < 0) || (index >= indices.size())) {
      ...
      if (pColName == NULL) {
        throw UtilException("Error calling method col.__str__()",
                            "pyColDict_AsPairedVector", "DippyPythonUtils");
      }

      string name = PyBytes_AsString(
 ...
 */
int pyColDict_AsPackedArrays(PyObject *pColDict, map<PyObject *, int> indices,
                             int **inds, double **vals) {
  int len = PyObject_Length(pColDict);
  *inds = new int[len];
  *vals = new double[len];
    ...
    if ((index < 0) || (index >= indices.size())) {
      ...
      if (pColName == NULL) {
        throw UtilException("Error calling method col.getName()",
                            "pyColDict_AsPackedArrays", "DippyPythonUtils");
      }
...
CoinPackedMatrix *
pyConstraints_AsPackedMatrix(PyObject *pRowList,
 ...
 */
void insertTupleToPyList(PyObject *pList, unsigned position, PyObject *key,
                         PyObject *value) {
  PyObject *pTuple = PyTuple_New(2);
  PyTuple_SetItem(pTuple, 0, key);
  PyTuple_SetItem(pTuple, 1, value);
