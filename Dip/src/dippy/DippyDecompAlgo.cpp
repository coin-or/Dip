 ...
 * Perform branching
 *
 * This function should populate the (down|up)Branch(LB|UB) vectors with
 * (indices, bound-value) pairs which define the branch.
 */
bool DippyAlgoMixin::chooseBranchSet(
    DecompAlgo *algo, std::vector<std::pair<int, double>> &downBranchLB,
  ...
  if (pResult == Py_None) {
    ...
    ret_val = algo->DecompAlgo::chooseBranchSet(downBranchLB, downBranchUB,
                                                upBranchLB, upBranchUB);

    // Original comment: No branching set was returned. This shouldn't happen
    // tkr 11/11/15: Actually, it can happen if the solution is integral, but
    // not feasible. This happens sometimes when column generation is halted
    // because of tailoff and the solution to the relaxation is feasible. I'm
    // leaving the commented code here for posterity
    // assert(ret_val == true);
    ...
    // or else, the function should have returned 4 lists of (name, value)
    // tuples
...
void DippyAlgoMixin::postProcessBranch(DecompAlgo *algo,
  ...
  addTupleToPyList(pOutput, PyUnicode_FromString("nodeIndex"),
                   PyLong_FromLong(algo->getNodeIndex()));
  ...
  if (pResult == NULL) {
    throw UtilException("postProcessNode call failed.", "postProcessNode",
                        "DippyAlgoMixin");
  }
