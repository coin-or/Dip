...
void DippyDecompApp::createModels() {
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    if (pRowName == NULL) {
      throw UtilException("Error calling method row.getName()", "createModels",
                          "DippyDecompApp");
    }
    char getLb[] = "getLb";
    ...
    if (pRowLb == NULL) {
      throw UtilException("Error calling method row.getLb()", "createModels",
                          "DippyDecompApp");
    }
    char getUb[] = "getUb";
    ...
    if (pRowUb == NULL) {
      throw UtilException("Error calling method row.getUb()", "createModels",
                          "DippyDecompApp");
    }

    name = PyBytes_AsString(
        PyUnicode_AsEncodedString(pRowName, "UTF-8", "strict"));
    ...
    m_rowIndices[pRow] = i; // Don't need to increase reference count here as
                            // m_rowList references pRow
  }
  ...
  for (int j = 0; j < m_numCols; j++) {
    ...
    if (pColName == NULL) {
      throw UtilException("Error calling method col.getName()", "createModels",
                          "DippyDecompApp");
    }
    char getLb[] = "getLb";
    ...
    if (pColLb == NULL) {
      throw UtilException("Error calling method col.getLb()", "createModels",
                          "DippyDecompApp");
    }
    char getUb[] = "getUb";
    ...
    if (pColUb == NULL) {
      throw UtilException("Error calling method col.getUb()", "createModels",
                          "DippyDecompApp");
    }
    ...
    name = PyBytes_AsString(
        PyUnicode_AsEncodedString(pColName, "UTF-8", "strict"));
  ...
  modelCore->M =
      pyConstraints_AsPackedMatrix(pRowList, m_rowIndices, m_colIndices);
  ...
  for (int p = 0; p < nRelaxed; p++) {
    ...
    char O[] = "O";
    PyObject *pRelaxAsTuple =
        PyObject_CallMethod(m_pProb, getRelaxAsTuple, O, pRelax);
    ...
    modelRelax->M =
        pyConstraints_AsPackedMatrix(pRowList, relaxRowIndices, m_colIndices);
...
DecompSolverStatus DippyDecompApp::solveRelaxed(const int whichBlock,
  ...
  char OOd[] = "OOd";
  PyObject *pStatandVarList = PyObject_CallMethod(
      m_pProb, solveRelaxed, OOd, pRelaxKey, pRedCostList, pConvexDual);
  ...
  // In the new design, we need to allow the possibility that the user will
  // solve the problem exactly, but not find any solutions with reduced costs
  // zero The below is is commented out and left in the source for posterity tkr
  // 11/11/15
  // if (nVars == 0) {
  ...
  // solveRelaxed returns 3-tuples (cost, reduced cost, dictionary of (variable,
  // value) pairs) We can use these to construct a C++ DecompVar objects
  double cost, rc;
 ...
 */
bool DippyDecompApp::APPisUserFeasible(const double *x, const int n_cols,
  ...
  char Od[] = "Od";
  PyObject *pResult =
      PyObject_CallMethod(m_pProb, isUserFeasible, Od, pSolutionList, pTolZero);

  if (pResult == NULL) {
    throw UtilException("Error calling method prob.isUserFeasible()",
                        "APPisUserFeasible", "DippyDecompApp");
  }

  // This should not happen as having isUserFeasible present but not returning a
  // boolean is not good
  if (pResult == Py_None) {
...
int DippyDecompApp::generateCuts(const double *x, DecompCutList &cutList) {
  ...
  // PyObject *pSolutionList = pyTupleList_FromDoubleArray(x, m_colList);
  // MO (28/2/2012) - Don't need this anymore as solution is contained within
  // node
  PyObject *pPackagedNode =
      pyTupleList_FromNode(getDecompAlgo(), STAT_FEASIBLE);
  char generateCuts[] = "generateCuts";
  char O[] = "O";
  PyObject *pCutList =
      PyObject_CallMethod(m_pProb, generateCuts, O, pPackagedNode);

  if (pCutList == NULL) {
    throw UtilException("Error calling method prob.generateCuts()",
                        "generateCuts", "DippyDecompApp");
  }
  ...
  // generateCuts returns constraints, i.e., dictionary of (variable, value)
  // pairs also with name, lb, ub
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (pLb == NULL) {
      throw UtilException("Error calling method row.getLb()", "generateCuts",
                          "DippyDecompApp");
    }
    char getUb[] = "getUb";
    ...
    if (pUb == NULL) {
      throw UtilException("Error calling method row.getUb()", "generateCuts",
                          "DippyDecompApp");
    }
    ...
    double *varVals = NULL;
    int numPairs =
        pyColDict_AsPackedArrays(pRow, m_colIndices, &varInds, &varVals);
    assert(numPairs == PyObject_Length(pRow));
    // arrays are now owned by the Cut object
    DippyDecompCut *cut =
        new DippyDecompCut(lb, ub, numPairs, varInds, varVals);
 ...
 * Called by DIP, we interface with Python
 */
int DippyDecompApp::APPheuristics(const double *xhat, const double *origCost,
  ...
  char OO[] = "OO";
  PyObject *pSolList =
      PyObject_CallMethod(m_pProb, solveHeuristics, OO, pSolution, pObjective);

  if (pSolList == NULL) {
    throw UtilException("Error calling method prob.solveHeuristics()",
                        "APPheuristics", "DippyDecompApp");
  }
  ...
  for (int i = 0; i < len; i++) {
    ...
    double *varVals = NULL;
    int numPairs =
        pyColDict_AsPackedArrays(pSolution, m_colIndices, &varInds, &varVals);
...
int DippyDecompApp::generateInitVars(DecompVarList &initVars) {
  ...
  if (pVarList == NULL) {
    throw UtilException("Error calling method prob.generateInitVars()",
                        "generateInitVars", "DippyDecompApp");
  }
  ...
  int nVars = PyObject_Length(pVarList);
  // generateInitVars returns 2-tuples (index, (cost, dictionary of (variable,
  // value) pairs)) We can use these to construct a C++ DecompVar objects
  double cost;
  PyObject *pColDict;
  ...
  for (int i = 0; i < nVars; i++) {
    ...
    int numPairs = pyColDict_AsPackedArrays(pColDict, m_colIndices, &varInds,
                                            &varVals, varType);
