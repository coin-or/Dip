...
DLLEXPORT PyObject *Solve(PyObject *self, PyObject *args) {
  ...
  try {
    ...
    while (PyDict_Next(pParamDict, &pos, &pKey, &pValue)) {
      ...
      if (pSection != Py_None) {
        section = PyBytes_AsString(PyUnicode_AsEncodedString(
            PyTuple_GetItem(pKey, 0), "UTF-8", "strict"));
      }

      const char *name = PyBytes_AsString(PyUnicode_AsEncodedString(
          PyTuple_GetItem(pKey, 1), "UTF-8", "strict"));
      const char *value = PyBytes_AsString(
          PyUnicode_AsEncodedString(pValue, "UTF-8", "strict"));
