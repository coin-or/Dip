...
def Solve(prob, params=None):
        ...
        if isinstance(value, dict):
            ...
            for name, param_val in list(value.items()):
                if not isinstance(param_val, valid_types):
                    raise DipError(
                        "Bad value '%s' in parameter dictionary, expecting string or number"
                        % param_val
                    )
            ...
            raise DipError(
                "Bad value '%s' in parameter dictionary, expecting string" % value
            )

    # DIP only solves minimisation problems
    if prob.sense == pulp.LpMaximize:
        raise DipError(
            "DIP assumes a minimize objective, but DipProblem has "
            + "maximize objective.\n"
    ...
    for v in prob.variables():
        if v.lowBound < 0:
            raise DipError(
                "Variable %s has negative lower bound, please "
                + "re-formulate using sum of non-negative variables" % v.name
            )
    ...
    try:
        ...
        if prob.display_mode == "svg" and gimpy_installed:
            if prob.display_interval is not None:
                prob.Tree.write_as_svg(
                    filename="%s_%d" % (prob.svg_prefix, prob.last_svg + 1),
...
def createBranchLabel(lbs, ubs):
    ...
    for l in both:
        addStr = (
            str(int(lbs[l]))
            + "<="
            + "".join(x for x in str(l) if x not in "()_")
            + "<="
            + str(int(ubs[l]))
        )
        if first:
    ...
    for l in lbOnly:
        addStr = str(int(lbs[l])) + "<=" + "".join(x for x in str(l) if x not in "()_")
        if first:
            newWidth = len(addStr) + currWidth
        else:
            newWidth = len(addStr) + currWidth + 2
    ...
    for l in ubOnly:
        addStr = "".join(x for x in str(l) if x not in "()_") + "<=" + str(int(ubs[l]))
        if first:
            newWidth = len(addStr) + currWidth
        else:
            newWidth = len(addStr) + currWidth + 2
...
class DipProblem(pulp.LpProblem, DipAPI):
    ...
    def writeFull(self, instancefile, blockfile, mip=True):
        ...
        for r in self.relaxation.dict:
            ...
            for k in self.relaxation.dict[r].constraints:
                f.write(
                    self.relaxation.dict[r]
                    .constraints[k]
                    .asCplexLpConstraint(str(k) + "_" + rname)
                )
        ...
        if repeated_names:
            raise PulpError(
                "Repeated variable names in Lp format\n" + str(repeated_names)
            )
        ...
        if mip:
            vg = [
                v
                for v in vs
                if not (v.isPositive() and v.cat == pulp.LpContinuous)
                and not v.isBinary()
            ]
        else:
    ...
    def chooseBranchSet(self, xhat):
        ...
        try:
            ...
            if len(diff) > 0:
                raise DipError(
                    "Solution and variable list don't match in chooseBranchSet"
                )
            ...
            if (branch_sets[0] or branch_sets[1]) and (
                branch_sets[2] or branch_sets[3]
            ):
                return branch_sets
            else:
                raise DipError(
                    "Invalid bounds returned from user-specified branch_method"
                )
        except Exception as ex:
            ...
            if gimpy_installed:
                ...
                if parentInd == -1:
                    ...
                    if self.Tree.attr["display"] == "svg":
                        if self.display_interval is not None:
                            if numNodes % self.display_interval in [0, 1]:
                                self.Tree.write_as_svg(
                                    filename="%s_0" % self.svg_prefix,
                    ...
                    if edge_label is not None:
                        self.Tree.set_edge_attr(parentInd, nodeInd, "label", edge_label)
                    if self.Tree.attr["display"] == "svg":
                        if self.display_interval is not None:
                            if numNodes % self.display_interval in [0, 1]:
                                self.Tree.write_as_svg(
                                    filename="%s_%d"
                                    % (self.svg_prefix, self.last_svg + 1),
                                    prevfile="%s_%d" % (self.svg_prefix, self.last_svg),
                                    nextfile="%s_%d"
                                    % (self.svg_prefix, self.last_svg + 2),
                                    highlight=nodeInd,
                                )
                                self.last_svg += 1
                if self.display_interval is not None:
            ...
            if gimpy_installed:
                ...
                for n in outputDict:
                    if n == "pDownUB":
                        ...
                        self.Tree.set_edge_attr(nodeInd, -numNodes, "label", labelStr)
                        numNodes += 1
    ...
    def solveRelaxed(self, key, redCostX, target):
        ...
        try:
            ...
            if len(diff) > 0:
                print(diff)
                raise DipError(
                    "Reduced cost and variable list don't match in", "solveRelaxed"
                )
            ...
            if len(dvs) > 0:
                ...
                for var in dvs:
                    if isinstance(var, dict):
                        cost = sum(
                            self.objective[i] * var[i]
                            for i in var
                            if i in self.objective
                        )
                        red_cost = sum(
                            redCostDict[i] * var[i] for i in var if i in redCostDict
                        )
    ...
    def isUserFeasible(self, sol, tol):
        ...
        try:
            ...
            if len(diff) > 0:
                raise DipError(
                    "Solution and variable list don't match in isUserFeasible"
                )
    ...
    def solveHeuristics(self, xhat, costX):
        ...
        try:
            ...
            if len(diff) > 0:
                raise DipError(
                    "Solution and variable list don't match in solveHeuristics"
                )
