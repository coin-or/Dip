...
def draw(path, mytype, id):
    ...
    canvas = Canvas(
        root, width=scale + (delta * 2), height=scale + (delta * 2), bg="white"
    )
    ...
    for _k_ in range(len(P)):
        ...
        for j in V:
            if j[2] == k[1]:
                if j[0] == 0:
                    canvas.create_line(
                        k[3] + 10, k[4] + 10, j[3] + 7, j[4] + 7, dash=(3, 3)
                    )
                else:
                    canvas.create_line(
                        k[3] + 10,
                ...
                if j[1] > 9:
                    ...
                    _d_ = 0
                canvas.create_oval(
                    j[3], j[4], j[3] + 15, j[4] + 15, width=1, fill=C[_k_]
                )
        ...
        canvas.create_rectangle(k[3], k[4], k[3] + 20, k[4] + 20, width=2, fill=C[_k_])
