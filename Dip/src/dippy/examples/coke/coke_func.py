...
class CokeProb(object):
    def __init__(
        self, supply, demand, LOCATIONS, build_costs, conversion_factor, transport_costs
    ):
...
def formulate(cp):
    ...
    # objective
    prob += (
        1e6 * lpSum(buildVars[(l, s)] * cp.build_costs[s] for (l, s) in LOC_SIZES)
        + lpSum(flowVars[(s, d)] * cp.transport_costs[(s, d)] for (s, d) in cp.ARCS),
        "min",
    )
    ...
    for loc in cp.LOCATIONS:
        prob += lpSum(flowVars[(loc, i)] for i in cp.CUSTOMERS) <= lpSum(
            buildVars[(loc, s)] * s for s in cp.SIZES
        )
    ...
    for m in cp.MINES:
        prob += lpSum(flowVars[(m, j)] for j in cp.LOCATIONS) <= cp.supply[m]

    # conserve flow (locations)
    # convert from coal to coke
    for loc in cp.LOCATIONS:
        prob += (
            lpSum(flowVars[(m, loc)] for m in cp.MINES)
            - cp.conversion_factor * lpSum(flowVars[(loc, c)] for c in cp.CUSTOMERS)
            >= 0
        )

    for c in cp.CUSTOMERS:
        prob += lpSum(flowVars[(loc, c)] for loc in cp.LOCATIONS) >= cp.demand[c]
...
def do_branch(prob, sol):
    ...
    for loc in LOCATIONS:
        ...
        if bigger - sol_size > tol and sol_size - smaller > tol:
            ...
            up_ubs = dict([(buildVars[loc, SIZES[j]], 0) for j in range(0, i)])
