...
MINE_SUPPLY = {"M1": 25.8, "M2": 728, "M3": 1456, "M4": 49, "M5": 36.9, "M6": 1100}
MINES = list(MINE_SUPPLY.keys())
MINES.sort()

LOCATIONS = ["L1", "L2", "L3", "L4", "L5", "L6"]

SIZE_COSTS = {0: 0, 75: 4.4, 150: 7.4, 225: 10.5, 300: 13.5, 375: 16.5, 450: 19.6}
SIZES = list(SIZE_COSTS.keys())
SIZES.sort()

CUSTOMER_DEMAND = {"C1": 83, "C2": 5.5, "C3": 6.975, "C4": 5.5, "C5": 720.75, "C6": 5.5}
...
CUST_TRANS = read_table(CUST_TRANS_DATA, int, transpose=True)
...
# create variables
buildVars = LpVariable.dicts("Build", LOC_SIZES, None, None, LpBinary)
...
# objective
prob += (
    1e6 * lpSum(buildVars[(l, s)] * SIZE_COSTS[s] for (l, s) in LOC_SIZES)
    + lpSum(flowVars[(s, d)] * ARC_COSTS[(s, d)] for (s, d) in ARCS),
    "min",
)

# plant availability
for loc in LOCATIONS:
    prob += lpSum(flowVars[(loc, i)] for i in CUSTOMERS) <= lpSum(
        buildVars[(loc, s)] * s for s in SIZES
    )
...
for m in MINES:
    prob += lpSum(flowVars[(m, j)] for j in LOCATIONS) <= old_div(MINE_SUPPLY[m], CC)

for loc in LOCATIONS:
    prob += (
        lpSum(flowVars[(m, loc)] for m in MINES)
        - lpSum(flowVars[(loc, c)] for c in CUSTOMERS)
        >= 0
    )

for c in CUSTOMERS:
    prob += lpSum(flowVars[(loc, c)] for loc in LOCATIONS) >= CUSTOMER_DEMAND[c]
...
def do_branch(prob, sol):
    ...
    for loc in LOCATIONS:
        sol_size = sum(sol[buildVars[loc, size]] * size for size in SIZES)
        # smallest index and size larger than sol_size
        if abs(sol_size - SIZES[-1]) < tol:
            continue
        i, bigger = [(i, s) for i, s in enumerate(SIZES) if s > sol_size][0]
        if i == 0:
            smaller = 0
        else:
            smaller = SIZES[i - 1]
        if bigger - sol_size > tol and sol_size - smaller > tol:
            down_branch_ub = dict(
                [(buildVars[loc, SIZES[j]], 0) for j in range(i, len(SIZES))]
            )
            up_branch_ub = dict([(buildVars[loc, SIZES[j]], 0) for j in range(0, i)])
...
def print_var_table(rows, cols, var, fn=lambda x: x):
    print_table(rows, cols, lambda x, y: fn(var[(x, y)].varValue))


for (l, s) in LOC_SIZES:
    if buildVars[(l, s)].varValue > 0:
        print("Build %s %s (%s)" % (l, s, buildVars[(l, s)].varValue))
print()

print_var_table(MINES, LOCATIONS, flowVars, fn=lambda x: CC * x)
print()
