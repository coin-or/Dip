...
def formulate(bpp):
    ...
    assign_vars = LpVariable.dicts(
        "x", [(i, j) for i in bpp.ITEMS for j in bpp.BINS], cat=LpBinary
    )
    use_vars = LpVariable.dicts("y", bpp.BINS, cat=LpBinary)
    waste_vars = LpVariable.dicts(
        "w", [(j, k) for j in bpp.BINS for k in bpp.LIMITS], 0, None
    )
    ...
    for j in bpp.BINS:
        for k in bpp.LIMITS:
            prob += (
                lpSum(bpp.volume[i, k] * assign_vars[i, j] for i in bpp.ITEMS)
                + waste_vars[j, k]
                == bpp.capacity[k] * use_vars[j]
            )
