...
assign_vars = LpVariable.dicts(
    "AtLocation",
    [(i, j) for i in data.LOCATIONS for j in data.PRODUCTS],
    0,
    1,
    LpBinary,
)
use_vars = LpVariable.dicts("UseLocation", data.LOCATIONS, 0, 1, LpBinary)
waste_vars = LpVariable.dicts("Waste", data.LOCATIONS, 0, data.CAPACITY)
...
for i in LOCATIONS:
    prob.relaxation[i] += (
        lpSum(assign_vars[(i, j)] * REQUIREMENT[j] for j in PRODUCTS) + waste_vars[i]
        == CAPACITY * use_vars[i]
    )
...
def choose_antisymmetry_branch(prob, sol):
    ...
    if (up - num_locations > tol) and (num_locations - down > tol):  # Is fractional?
        # Down branch: provide upper bounds, lower bounds are default
        down_branch_ub = dict(
            [(use_vars[LOCATIONS[n]], 0) for n in range(int(down), len(LOCATIONS))]
        )
        # Up branch: provide lower bounds, upper bounds are default
        up_branch_lb = dict([(use_vars[LOCATIONS[n]], 1) for n in range(0, int(up))])
...
def solve_subproblem(prob, index, redCosts, target):
    ...
    for j in PRODUCTS:
        effs[j] = (
            redCosts[assign_vars[(loc, j)]] - redCosts[waste_vars[loc]] * REQUIREMENT[j]
        )
