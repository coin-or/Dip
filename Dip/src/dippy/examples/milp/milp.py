...
def GenerateRandomBlock(
    ...
    MAT = dict(
        ((i, j), random.randint(1, maxConsCoeff) if random.random() <= density else 0)
    ...
    RHS = dict(
        (
            i,
            random.randint(
                int(numVars * density * maxConsCoeff / tightness),
...
# Blocks
for k in range(numBlocks):
    OBJ, MAT, RHS = GenerateRandomBlock(
        [(k, i) for i in range(numBlockVars[k])], CONSTRAINTS[k]
    )
    for j in CONSTRAINTS[k]:
        prob.relaxation[k] += (
            lpSum([MAT[(k, i), j] * var[k, i] for i in range(numBlockVars[k])])
            <= RHS[j],
            j,
        )
