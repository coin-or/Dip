...
x = pulp.LpVariable.dicts(
    "possible_seatings", possible_seatings, lowBound=0, upBound=1, cat=pulp.LpInteger
)

seating_model = dippy.DipProblem(
    "Wedding Seating Model (DIP)",
    pulp.LpMinimize,
    display_mode="xdot",
    display_interval=0,
)
...
for table in tables:
    seating_model.relaxation[table] += (
        sum([x[(guest, table)] for guest in guests]) <= max_table_size,
...
for guest in guests:
    seating_model += (
        sum([x[(guest, table)] for table in tables]) == 1,
        "Must_seat_%s" % guest,
    )
...
happy = pulp.LpVariable.dicts(
    "table_happiness", tables, lowBound=0, upBound=None, cat=pulp.LpContinuous
)
...
for table in tables:
    for (a, b) in possible_pairs:
        seating_model.relaxation[table] += happy[table] >= (
...
def relaxed_solver(prob, table, redCosts, target):
    ...
    """
    dvs = []
    neg_guests = [g for g in guests if redCosts[x[(g, table)]] < 0.0]
    neg_guests.sort()
    # find all possible tables between two end points
    for pos1, pos2 in [
        (i, j) for i in range(len(neg_guests)) for j in range(len(neg_guests)) if j > i
    ]:
        # find the suitable guests that can be included in between the end
        # points
        candidate_guests = [
            (redCosts[x[(g, table)]], g) for g in neg_guests[pos1 + 1 : pos2]
        ]
        candidate_guests.sort()
        # pick the best guests (ie those with the negative reduced costs)
        possible_table_inner = [g for _, g in candidate_guests[: max_table_size - 2]]
        # This is the best table between the end points
        possible_table = [neg_guests[pos1]] + possible_table_inner + [neg_guests[pos2]]
        ...
        var_values = [(x[(g, table)], 1) for g in possible_table]
...
if seating_model.display_mode != "off":
    ...
    if seating_model.Tree.attr["display"] == "svg":
        seating_model.Tree.write_as_svg(
            filename="facility_node%d" % (numNodes + 1),
