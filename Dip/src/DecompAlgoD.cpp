...
// ------------------------------------------------------------------------- //
void DecompAlgoD::phaseUpdate(DecompPhase &phase, DecompStatus &status) {
    ...
    for (; it != m_phaseIObj.rend(); it++) {
      ...
      UTIL_DEBUG(m_param.LogDebugLevel, 3,
                 (*m_osLog) << setw(10) << "prevBound=" << setw(10)
                            ...
                            << ", thisBound=" << setw(10) << UtilDblToStr((*it))
                            << endl;);
      sumDiff += diff;
      prevBound = (*it);
      len++;
...
void DecompAlgoD::masterMatrixAddArtCols(CoinPackedMatrix *masterM,
  ...
  DecompColType colTypeL =
      origOrBranch == 'O' ? DecompCol_ArtForRowL : DecompCol_ArtForBranchL;
  DecompColType colTypeG =
      origOrBranch == 'O' ? DecompCol_ArtForRowG : DecompCol_ArtForBranchG;
  ...
  for (r = startRow; r < endRow; r++) {
    ...
    masterMatrixAddArtCol(colBeg, colInd, colVal, 'L', r, colIndex, colTypeL,
                          colLB[colIndex], colUB[colIndex], objCoeff[colIndex]);
...
void DecompAlgoD::createMasterProblem(DecompVarList &initVars) {
  ...
  int nColsMax =
      static_cast<int>(initVars.size()) + 2 * (m_nRowsOrig + m_nRowsBranch);
  ...
  */
  ...
  if (modelCore->colNames.size() > 0) {
    ...
    m_masterSI->setRowNames(modelCore->colNames, 0,
                            static_cast<int>(modelCore->colNames.size()), 0);
    ...
    m_masterSI->setRowNames(conRowNames, 0,
                            static_cast<int>(conRowNames.size()),
                            static_cast<int>(modelCore->colNames.size()));
  }

  if (colNames.size() > 0)
    m_masterSI->setColNames(colNames, 0, static_cast<int>(colNames.size()), 0);
