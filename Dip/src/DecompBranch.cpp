...
// --------------------------------------------------------------------- //
bool DecompAlgo::chooseBranchSet(
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "chooseBranchSet()",
  ...
  if (branchedOnIndex != -1) {
    ...
    downBranchUB.push_back(
        std::pair<int, double>(branchedOnIndex, floor(branchedOnValue)));
    upBranchLB.push_back(
        std::pair<int, double>(branchedOnIndex, ceil(branchedOnValue)));
    UTIL_MSG(
        m_param.LogDebugLevel, 3,
        int nColNames = static_cast<int>(modelCore->colNames.size());
        (*m_osLog) << "branchOnInd = " << branchedOnIndex << " -> ";

        if (branchedOnIndex < nColNames && branchedOnIndex >= 0)(*m_osLog)
        << modelCore->colNames[branchedOnIndex];
        else { m_app->printOriginalColumn(branchedOnIndex, m_osLog); }(*m_osLog)
        << "\tbranchOnVal = " << branchedOnValue << "\n";);
    return true;
  } else {
    return false;
  }
