...
AlpsTreeNode *AlpsDecompTreeNode::createNewTreeNode(AlpsNodeDesc *&desc) const {
  ...
  //---
  AlpsDecompModel *model = dynamic_cast<AlpsDecompModel *>(desc->getModel());
  AlpsDecompParam &param = model->getParam();
  UtilPrintFuncBegin(&cout, m_classTag, "createNewTreeNode()", param.msgLevel,
                     3);
  AlpsDecompTreeNode *node = new AlpsDecompTreeNode();
  node->desc_ = desc;
  UtilPrintFuncEnd(&cout, m_classTag, "createNewTreeNode()", param.msgLevel, 3);
  return node;
}
...
bool AlpsDecompTreeNode::checkIncumbent(AlpsDecompModel *model,
  ...
  UTIL_DEBUG(model->getParam().msgLevel, 3,
             cout << "DecompAlgo found IP incum = " << UtilDblToStr(candidateUB)
  ...
  if (candidateUB < currentUB) {
    ...
    //---
    AlpsDecompSolution *alpsDecompSol = new AlpsDecompSolution(
        decompSol->getSize(), decompSol->getValues(), decompSol->getQuality(),
        decompAlgo->getDecompApp(), getIndex(), getDepth());
    getKnowledgeBroker()->addKnowledge(AlpsKnowledgeTypeSolution, alpsDecompSol,
                                       candidateUB);
    //---
...
//===========================================================================//
int AlpsDecompTreeNode::process(bool isRoot, bool rampUp) {
  //---
  ...
  UtilPrintFuncBegin(&cout, m_classTag, "process()", param.msgLevel, 3);
  UTIL_DEBUG(param.msgLevel, 3,
             cout << "Start process of node: " << getIndex()
  ...
  if ((parentObjValue - primalTolerance) > currentUB) {
    doFathom = true;
    UTIL_DEBUG(param.msgLevel, 3,
               cout << "Fathom since parentObjValue=" << setw(10)
                    << UtilDblToStr(parentObjValue) << " currentUB = "
                    << setw(10) << UtilDblToStr(currentUB) << endl;);
    goto TERM_PROCESS;
  }
  ...
  if (!isRoot) {
    ...
    //---
    UTIL_MSG(
        param.msgLevel, 3, int c; double diffLB; double diffUB;
        ...
        for (c = 0; c < n_cols; c++) {
          ...
          if (!UtilIsZero(diffLB) || !UtilIsZero(diffUB)) {
            ...
            cout << "\t(lb,ub): (" << colLBCore[c] << "," << colUBCore[c]
                 << ")\t->\t(" << lbs[c] << "," << ubs[c] << ")" << endl;
          }
        });
  ...
  if (!isRoot) {
    ...
    if (gap <= relTolerance) {
      ...
      UTIL_MSG(param.msgLevel, 3,
               cout << "Fathom Node " << getIndex()
                    << " since globalLB= " << setw(10) << UtilDblToStr(globalLB)
  ...
  case STAT_IP_FEASIBLE:
    ...
    if (quality_ >= currentUB) {
      doFathom = true;
      UTIL_DEBUG(param.msgLevel, 3,
                 cout << "Fathom since thisQuality= " << setw(10)
                      << UtilDblToStr(thisQuality) << " quality_= " << setw(10)
                      << UtilDblToStr(quality_) << " currentUB = " << setw(10)
                      << UtilDblToStr(currentUB) << " gap = " << setw(10)
                      << UtilDblToStr(gap) << endl;);
    }

    UTIL_MSG(param.msgLevel, 3,
             cout << "Node " << getIndex() << " quality "
                  << UtilDblToStr(quality_) << " currentUB "
                  << UtilDblToStr(currentUB) << " doFathom " << doFathom
                  << endl;);
    break;
  case STAT_INFEASIBLE:
    ...
    doFathom = true;
    UTIL_MSG(param.msgLevel, 3, cout << "Fathom since node infeasible\n";);
    break;
  default:
    assert(0);
  }
  ...
  UTIL_MSG(param.msgLevel, 3,
           cout << "Node " << getIndex() << " bestQuality "
                << UtilDblToStr(quality_) << " bestFeasible "
                << UtilDblToStr(currentUB) << endl;);
TERM_PROCESS:
  ...
  UtilPrintFuncEnd(&cout, m_classTag, "process()", param.msgLevel, 3);
  return status;
}
...
int AlpsDecompTreeNode::chooseBranchingObject(AlpsModel *model) {
  ...
  UtilPrintFuncBegin(&cout, m_classTag, "chooseBranchingObject()",
                     param.msgLevel, 3);
  bool gotBranch = m->getDecompAlgo()->chooseBranchSet(
      downBranchLB_, downBranchUB_, upBranchLB_, upBranchUB_);
  ...
  UtilPrintFuncEnd(&cout, m_classTag, "chooseBranchingObject()", param.msgLevel,
                   3);
...
AlpsDecompTreeNode::branch() {
  ...
  if ((downBranchLB_.size() + downBranchUB_.size() == 0) ||
    ...
    throw CoinError("empty branch variable set(s)", "branch",
                    "AlpsDecompTreeNode");
  }
  ...
  for (unsigned i = 0; i < downBranchLB_.size(); i++) {
    if ((downBranchLB_[i].first < 0) || (downBranchLB_[i].first >= numCols)) {
      std::cout << "AlpsDecompError: downBranchLB_[" << i
                << "] variable = " << downBranchLB_[i].first
                << "; numCols = " << numCols << "; index_ = " << index_
                << std::endl;
      throw CoinError("branch index is out of range", "branch",
                      "AlpsDecompTreeNode");
    }
  ...
  for (unsigned i = 0; i < downBranchUB_.size(); i++) {
    if ((downBranchUB_[i].first < 0) || (downBranchUB_[i].first >= numCols)) {
      std::cout << "AlpsDecompError: downBranchUB_[" << i
                << "] variable = " << downBranchUB_[i].first
                << "; numCols = " << numCols << "; index_ = " << index_
                << std::endl;
      throw CoinError("branch index is out of range", "branch",
                      "AlpsDecompTreeNode");
  ...
  newNodes.push_back(CoinMakeTriple(static_cast<AlpsNodeDesc *>(child),
                                    AlpsNodeStatusCandidate, objVal));
  //---
  //--- Branch up
  //---
  ...
  for (unsigned i = 0; i < upBranchLB_.size(); i++) {
    if ((upBranchLB_[i].first < 0) || (upBranchLB_[i].first >= numCols)) {
      std::cout << "AlpsDecompError: upBranchLB_[" << i
                << "] variable = " << upBranchLB_[i].first
                << "; numCols = " << numCols << "; index_ = " << index_
                << std::endl;
      throw CoinError("branch index is out of range", "branch",
                      "AlpsDecompTreeNode");
  ...
  for (unsigned i = 0; i < upBranchUB_.size(); i++) {
    if ((upBranchUB_[i].first < 0) || (upBranchUB_[i].first >= numCols)) {
      std::cout << "AlpsDecompError: upBranchUB_[" << i
                << "] variable = " << upBranchUB_[i].first
                << "; numCols = " << numCols << "; index_ = " << index_
                << std::endl;
      throw CoinError("branch index is out of range", "branch",
                      "AlpsDecompTreeNode");
