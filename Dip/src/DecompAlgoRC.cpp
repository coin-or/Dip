...
DecompPhase DecompAlgoRC::phaseInit() {
  ...
  if (m_param.LogDumpModel > 1)
    printCurrentProblem(m_masterSI, "masterProb", m_nodeStats.nodeIndex,
    ...
    if (isIPFeasible(m_xhat)) {
      if (m_app->APPisUserFeasible(m_xhat, modelCore->getNumCols(),
        ...
        DecompSolution *decompSol = new DecompSolution(
            modelCore->getNumCols(), m_xhat, m_masterSI->getObjValue());
...
DecompStatus DecompAlgoRC::solutionUpdate(const DecompPhase phase,
  ...
  UTIL_DEBUG(m_app->m_param.LogDebugLevel, 3, (*m_osLog) << "\nVARS m_vars:\n";
             printVars(m_osLog););
  ...
  for (r = 0; r < n_coreRows; r++) {
    ...
    UTIL_DEBUG(m_app->m_param.LogDebugLevel, 3, (*m_osLog) << setprecision(8);
