...
//===========================================================================//
int DecompAlgoCGL::initGenerators(const int doClique, const int doOddHole,
                                  const int doFlowCover, const int doKnapCover,
                                  const int doMixIntRound, const int doGomory) {
...
int DecompAlgoCGL::generateCuts(OsiSolverInterface *cutGenSI,
                                OsiSolverInterface *masterSI, double *xhat,
  ...
  UTIL_MSG(m_logLevel, 3,
           (*m_logStream) << "Num clique     cuts= " << nCliqueCuts << endl
  ...
  UTIL_DEBUG(
      ...
      if (osiCuts.sizeColCuts() > 0) {
        (*m_logStream) << "WARNING: " << osiCuts.sizeColCuts()
