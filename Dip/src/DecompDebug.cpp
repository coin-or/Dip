...
bool DecompAlgo::checkPointFeasible(const DecompConstraintSet *model,
  ...
  for (i = 0; i < nCols; i++) {
    ...
    if (UtilIsZero(x[i], 1.0e-3) || (x[i] < 0 && UtilIsZero(colLB[i])) ||
    ...
    if (relViol > 0.0001) { // 0.01% violated
      ...
      (*m_osLog) << " LB= " << UtilDblToStr(colLB[i], precision)
                 ...
                 << " RelViol= " << UtilDblToStr(relViol, precision) << endl;
  ...
  for (i = 0; i < nRows; i++) {
    ...
    if (UtilIsZero(ax[i], 1.0e-3) || (ax[i] < 0 && UtilIsZero(rowLB[i])) ||
    ...
    if (relViol > 0.005) { // 0.5% violated
      ...
      if (relViol > 0.05) {
        ...
        if (UtilIsZero(ax[i], 1.0e-3) || (ax[i] < 0 && UtilIsZero(rowLB[i])) ||
          ...
          for (k = 0; k < numNZ; k++) {
            if (fabs(els[k]) > 1.0e7) {
              (*m_osLog) << "  row has a big coefficient " << els[k] << endl;
              isFeas = true;
              break;
            }
          }
...
void DecompAlgo::checkMasterDualObj() {
  ...
  UTIL_DEBUG(m_param.LogDebugLevel, 4,
             (*m_osLog) << "checkMasterDualObj" << setw(10) << "primalObj="
                        << setw(10) << UtilDblToStr(primalObj, 3) << setw(10)
                        << "dualObj=" << setw(10) << UtilDblToStr(dualObj, 3)
                        << endl;);
  ...
  if (relViol > 1.0e-4) {
    cerr << "checkMasterDualObj" << setw(10) << "primalObj=" << setw(10)
         << UtilDblToStr(primalObj, 3) << setw(10) << "dualObj=" << setw(10)
         << UtilDblToStr(dualObj, 3) << endl;
...
bool DecompAlgo::isDualRayInfProof(const double *dualRay,
                                   ...
                                   const double *rowRhs, ostream *os) {
  //---
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((ybPos && (yA[i] > 1.0e-2)) || (!ybPos && (yA[i] < -1.0e-2))) {
      if (os) {
  ...
  for (i = 0; i < m; i++) {
    ...
    for (int j = 0; j < lenI; j++) {
      ...
      printf("i: %d, j: %d, indIj: %d, elsIj: %g ray: %g yA_i: %g\n", i, j,
             indI[j], elsI[j], dualRay[indI[j]], yA_i);
    }
...
//===========================================================================//
void DecompAlgo::printBasisInfo(OsiSolverInterface *si, ostream *os) {
  int b, r, c;
  int *basics = 0;
  int *rstat = 0;
  int *cstat = 0;
...
void DecompAlgo::printCurrentProblemDual(OsiSolverInterface *si,
                                         ...
                                         const int nodeIndex, const int cutPass,
                                         const int pricePass) {
  if (!si) {
    return;
  }

  UtilPrintFuncBegin(m_osLog, m_classTag, "printCurrentProblemDual()",
                     m_param.LogDebugLevel, 2);
  if (m_param.DecompIPSolver == "CPLEX") {
#ifdef DIP_HAS_CPX
    OsiCpxSolverInterface *siCpx = dynamic_cast<OsiCpxSolverInterface *>(si);
    CPXENVptr env = siCpx->getEnvironmentPtr();
    CPXLPptr lp = siCpx->getLpPtr(OsiCpxSolverInterface::KEEPCACHED_ALL);
    string filename = DecompAlgoStr[m_algo] + "_" + baseName + ".n" +
                      UtilIntToStr(nodeIndex) + ".c" + UtilIntToStr(cutPass) +
                      ".p" + UtilIntToStr(pricePass) + ".dual.mps";
    double objShift;
    int status = CPXdualwrite(env, lp, filename.c_str(), &objShift);

    if (status)
      throw UtilException("CPXdualwrite failure", "printCurrentProblemDual",
                          "DecompAlgo");

    printf("objShift in dual = %g\n", objShift);
    UTIL_DEBUG(m_param.LogDebugLevel, 3,
               (*m_osLog) << "calling CPXdualwrite filename = " << filename
                          << endl;);
#endif
  }
  UtilPrintFuncEnd(m_osLog, m_classTag, "printCurrentProblemDual()",
                   m_param.LogDebugLevel, 2);
}

//===========================================================================//
void DecompAlgo::printCurrentProblem(const OsiSolverInterface *si,
                                     const string baseName, const int nodeIndex,
                                     const int cutPass, const int pricePass,
                                     const int blockId, const bool printMps,
                                     const bool printLp) {
  if (!si) {
    return;
  }
...
//===========================================================================//
void DecompAlgo::printCurrentProblem(const OsiSolverInterface *si,
                                     const string fileName, const bool printMps,
                                     const bool printLp) {
  if (!si) {
    return;
  }
  ...
  UTIL_DEBUG(m_param.LogDebugLevel, 3,
             ...
             if (printMps)(*m_osLog)
                 << "calling writeMps fileName = " << fileName << endl;
             if (printLp)(*m_osLog)
             << "calling writeLp  fileName = " << fileName << endl;);
  ...
  if (m_param.DecompIPSolver == "CPLEX") {
    ...
    if (printLp) {
      ...
      si->writeLpNative(fileNameLp.c_str(), rowNamesChar, colNamesChar, epsilon,
                        numberAcross, decimals);
    }
    ...
    if (printLp) {
      ...
      si->writeLp(fileName.c_str(), "lp", epsilon, numberAcross, decimals);
    }
  }

  UtilPrintFuncEnd(m_osLog, m_classTag, "printCurrentProblem()",
                   m_param.LogDebugLevel, 2);
}
...
void DecompAlgo::checkReducedCost(const double *u, const double *u_adjusted) {
  ...
  for (it = m_vars.begin(); it != m_vars.end(); it++) {
    ...
    UTIL_DEBUG(m_app->m_param.LogDebugLevel, 5,
               (*m_osLog) << "MasterColIndex = " << setw(6)
                          << (*it)->getColMasterIndex() << "Block = " << setw(3)
                          << b << "LPRedCost = " << setw(10)
                          ...
                          << "Alpha = " << setw(10) << UtilDblToStr(alpha, 5)
                          << endl;);
    //---
    //--- sanity check - none of the columns currently in master
    //---    should have negative reduced cost
    ...
    if (rcLPi < -m_param.RedCostEpsilon && colUB[index] > DecompEpsilon) {
      (*m_osLog) << "VAR v-index:" << var_index++
                 << " m-index: " << (*it)->getColMasterIndex()
                 << " b-index: " << b << " rcLP: " << rcLPi << endl;
    ...
    if (m_phase == PHASE_PRICE2) {
      ...
      if (!UtilIsZero(objCalc - objC[(*it)->getColMasterIndex()], 1.0e-3)) {
        (*m_osLog) << "VAR v-index:" << var_index++
                   << " m-index: " << (*it)->getColMasterIndex()
                   << " b-index: " << b
                   << " objLP: " << objC[(*it)->getColMasterIndex()]
                   << " objCalc: " << objCalc << endl;
        (*it)->print(m_infinity, m_osLog, modelCore->colNames,
    ...
    //---
    if (!m_param.DualStab &&
        !UtilIsZero(rcLP[(*it)->getColMasterIndex()] - (redCost - alpha),
                    1.0e-3)) {
      //---
      ...
      (*m_osLog) << "VAR v-index:" << var_index++
                 << " m-index: " << (*it)->getColMasterIndex()
                 << " b-index: " << b << " rc: " << redCost
                 << " alpha: " << alpha << " rc-a: " << redCost - alpha
                 << " RCLP: " << rcLP[(*it)->getColMasterIndex()] << endl;
      //---
      ...
      //---
      (*m_osLog) << " objLP: "
                 << UtilDblToStr(objC[(*it)->getColMasterIndex()], 4) << endl;
      ...
      if (m_algo != DECOMP) {
        ...
        for (r = 0; r < modelCore->getNumRows(); r++) {
          ...
          UtilPrintPackedVector(vec, m_osLog, modelCore->getColNames(), denseS);
        }
      ...
      CoinShallowPackedVector vec = Mc->getVector((*it)->getColMasterIndex());
      ...
      (*m_osLog) << " RC   : "
                 << UtilDblToStr(objC[(*it)->getColMasterIndex()] - uA, 4)
                 << endl;

      (*m_osLog) << " RCLP : "
                 << UtilDblToStr(rcLP[(*it)->getColMasterIndex()], 4) << endl;
...
DecompSolverResult *DecompAlgoC::solveDirect(const DecompSolution *startSol) {
  ...
  //---
  UtilPrintFuncBegin(m_osLog, m_classTag, "solveDirect()",
                     m_param.LogDebugLevel, 2);
  DecompVarList dummy;
  int i, nNodes;
  ...
  if (m_param.DecompIPSolver == "Cbc") {
    ...
    if (!UtilIsInSet(solStatus, statusSet, 2)) {
      cerr << "Error: CBC IP solver status = " << solStatus << endl;
    ...
    if (startSol) {
      ...
      status = CPXaddmipstarts(cpxEnv, cpxLp, 1, nCols, beg, varInd, solution,
                               NULL, NULL);

      if (status)
        throw UtilException("CPXaddmipstarts failure", "solveDirect",
                            "DecompAlgoC");
    ...
    //---
    status =
        CPXsetintparam(cpxEnv, CPX_PARAM_THREADS, m_param.NumThreadsIPSolver);

    if (status)
      throw UtilException("CPXsetdblparam failure", "solveDirect",
                          "DecompAlgoC");
    ...
    if (status)
      throw UtilException("CPXgetbestobjval failure", "solveDirect",
                          "DecompAlgoC");
    ...
    if (solStatus == CPXMIP_OPTIMAL || solStatus == CPXMIP_OPTIMAL_TOL ||
      ...
      if (status)
        throw UtilException("CPXgetmipobjval failure", "solveDirect",
                            "DecompAlgoC");
  ...
  (*m_osLog) << "DIRECT SOLVE"
             ...
             << " Nodes= " << setw(8) << nNodes << " objLB= " << setw(10)
             << UtilDblToStr(objLB, 3) << " objUB= " << setw(10)
             << UtilDblToStr(objUB, 3) << endl;
  UtilPrintFuncEnd(m_osLog, m_classTag, "solveDirect()", m_param.LogDebugLevel,
                   2);
  return result;
}
