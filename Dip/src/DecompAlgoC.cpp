...
//===========================================================================//
void DecompAlgoC::setMasterBounds(const double *lbs, const double *ubs) {
...
bool DecompAlgoC::updateObjBound(const double mostNegRC) {
  ...
  //---
  UtilPrintFuncBegin(m_osLog, m_classTag, "updateObjBoundLB()",
                     m_param.LogDebugLevel, 2);
  double thisBoundLB = m_masterSI->getObjValue();
  setObjBound(thisBoundLB, thisBoundLB);
  UTIL_DEBUG(m_param.LogDebugLevel, 5,
             (*m_osLog) << "ThisLB = " << UtilDblToStr(thisBoundLB) << "\t"
                        << "BestLB = "
                        << UtilDblToStr(m_nodeStats.objBest.first) << "\n";);
  UtilPrintFuncEnd(m_osLog, m_classTag, "updateObjBoundLB()",
                   m_param.LogDebugLevel, 2);
  return false;
}

//===========================================================================//
void DecompAlgoC::recomposeSolution(const double *solution, double *rsolution) {
...
//===========================================================================//
void DecompAlgoC::phaseUpdate(DecompPhase &phase, DecompStatus &status) {
  ...
  //---
  if ((m_param.TotalCutItersLimit == 0) || (m_param.RoundCutItersLimit == 0)) {
    nextPhase = PHASE_DONE;
