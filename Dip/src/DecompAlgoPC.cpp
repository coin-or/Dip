...
void DecompAlgoPC::phaseInit(DecompPhase &phase) {
  ...
  //---
  UtilPrintFuncBegin(m_osLog, m_classTag, "phaseInit()", m_param.LogDebugLevel,
                     2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "phaseInit()", m_param.LogDebugLevel,
                   2);
}
...
void DecompAlgoPC::adjustMasterDualSolution() {
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "adjustMasterDualSolution()",
  ...
  if (((m_nodeStats.cutCallsTotal + m_nodeStats.priceCallsTotal) == 0) ||
      m_firstPhase2Call) {
  ...
  if (m_param.LogDebugLevel >= 3) {
    ...
    for (r = 0; r < m_masterSI->getNumRows(); r++) {
      if (!(UtilIsZero(m_dual[r]) && UtilIsZero(m_dualRM[r]) &&
            UtilIsZero(m_dualST[r]))) {
        if (r < static_cast<int>(rowNames.size())) {
          (*m_osLog) << "MASTER " << DecompRowTypeStr[m_masterRowType[r]]
                     << " DUAL[ " << r << "->" << rowNames[r]
                     << "] = " << m_dual[r] << " RM = " << m_dualRM[r]
                     << " ST = " << m_dualST[r] << endl;
        } else
          (*m_osLog) << "MASTER " << DecompRowTypeStr[m_masterRowType[r]]
                     << " DUAL[ " << r << "] = " << m_dual[r]
                     << " RM = " << m_dualRM[r] << " ST = " << m_dualST[r]
                     << endl;
      }
    }
  }

  UtilPrintFuncEnd(m_osLog, m_classTag, "adjustMasterDualSolution()",
                   m_param.LogDebugLevel, 2);
}
...
int DecompAlgoPC::adjustColumnsEffCnt() {
  ...
  double redCostI = 0.0;
  UtilPrintFuncBegin(m_osLog, m_classTag, "adjustColumnsEffCnt()",
                     m_param.LogDebugLevel, 2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "adjustColumnsEffCnt()",
                   m_param.LogDebugLevel, 2);
  return status;
}
...
int DecompAlgoPC::compressColumns() {
  ...
  //---
  int status = DecompStatOk;
  UtilPrintFuncBegin(m_osLog, m_classTag, "compressColumns()",
                     m_param.LogDebugLevel, 2);
  ...
  if (nHistorySize > 0) {
    ...
    if (masterUB > -m_infinity && masterUB < m_infinity) {
      if (masterUB != 0.0) {
  ...
  int nColsSinceLast = nMasterCols - m_compressColsLastNumCols;

  int nIterSinceLast = m_nodeStats.priceCallsTotal - m_compressColsLastPrice;

  int nColsSinceLastLimit = static_cast<int>(
      ceil(m_compressColsLastNumCols * CompressColsSizeMultLimit));

  UTIL_MSG(
      m_param.LogLevel, 4,
      (*m_osLog) << "nMasterCols              = " << nMasterCols << endl;
      (*m_osLog) << "m_compressColsLastNumCols= " << m_compressColsLastNumCols
                 << endl;
      (*m_osLog) << "nColsSinceLast           = " << nColsSinceLast << endl;
      (*m_osLog) << "priceCallsTotal          = " << m_nodeStats.priceCallsTotal
                 << endl;
      (*m_osLog) << "m_compressColsLastPrice  = " << m_compressColsLastPrice
                 << endl;
      (*m_osLog) << "nItersSinceLast          = " << nIterSinceLast << endl;);
  ...
  CoinWarmStartBasis *warmStart = dynamic_cast<CoinWarmStartBasis *>(
      m_masterSI->getPointerToWarmStart(mustDeleteWS));
  ...
  UTIL_DEBUG(m_param.LogLevel, 5, (*m_osLog) << "VARS before compress:" << endl;
             printVars(m_osLog););
  ...
  while (li != m_vars.end()) {
    ...
    UTIL_DEBUG(m_param.LogLevel, 4,
               ...
               (*m_osLog) << "CompressCol"
                          ...
                          << " currSol= " << setw(10)
                          << UtilDblToStr(masterSolution[colMasterIndex], 3)
                          << endl;);
  ...
  if (lpColsToDelete.size() > 0) {
    ...
    UTIL_MSG(m_param.LogLevel, 3,
             (*m_osLog) << "Num Columns Deleted = " << lpColsToDelete.size()
                        << " Cols = " << nCols << " NoDel = " << nColsNoDel
                        << " Basic = " << nColsBasic
                        << " EffPos = " << nColsEffPos << endl;);
  ...
  UTIL_DELARR(basics);
  UTIL_DELARR(isBasic);
  UtilPrintFuncEnd(m_osLog, m_classTag, "compressColumns()",
                   m_param.LogDebugLevel, 2);
  return status;
}
...
void DecompAlgoPC::phaseDone() {
  UtilPrintFuncBegin(m_osLog, m_classTag, "phaseDone()", m_param.LogDebugLevel,
                     2);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "phaseDone()", m_param.LogDebugLevel,
                   2);
}
...
void DecompAlgoPC::solveMasterAsMIP() {
  ...
  UtilPrintFuncBegin(m_osLog, m_classTag, "solveMasterAsMIP()",
                     m_param.LogDebugLevel, 2);
  //---
  ...
  if (m_param.LogDumpModel >= 2)
    printCurrentProblem(m_masterSI, "masterProbRootIP", m_nodeStats.nodeIndex,
  ...
  if (m_param.DecompIPSolver == "SYMPHONY") {
    ...
    throw UtilException("Unknown solver selected", "solveMasterAsMIP",
                        "DecompAlgoPC");
  }
  ...
  if (result.m_nSolutions) {
    ...
    if (m_app->APPisUserFeasible(rsolution, modelCore->getNumCols(),
      ...
      if (isDup) {
        UTIL_MSG(m_param.LogLevel, 3,
                 (*m_osLog)
                     << "Solution is a duplicate, not pushing." << endl;);
      } else {
        DecompSolution *decompSol = new DecompSolution(
            modelCore->getNumCols(), rsolution, getOrigObjective());
  ...
  //---
  for (colIndex = 0; colIndex < nMasterCols; colIndex++) {
    if (isMasterColStructural(colIndex) || isMasterColMasterOnly(colIndex)) {
      m_masterSI->setContinuous(colIndex);
    }
  }

  UtilPrintFuncEnd(m_osLog, m_classTag, "solveMasterAsMIP()",
                   m_param.LogDebugLevel, 2);
}
...
void DecompAlgoPC::solveMasterAsMIPSym(DecompSolverResult *result) {
  ...
  osiSym->loadProblem(
      const_cast<CoinPackedMatrix &>(*m_masterSI->getMatrixByRow()),
  ...
  if ((status == PREP_OPTIMAL_SOLUTION_FOUND) ||
    ...
    if (status == FUNCTION_TERMINATED_ABNORMALLY)
      throw UtilException("sym_get_col_solution failure", "solveMasterAsMIP",
                          "DecompSubModel");
  } else {
  ...
  if (status == (TM_ERROR__USER || TM_ERROR__COMM_ERROR ||
    ...
    std::cerr << "Error: SYPHONMY IP solver status =  " << status << std::endl;
  }
...
void DecompAlgoPC::solveMasterAsMIPCbc(DecompSolverResult *result) {
  ...
  cbc.setDblParam(CbcModel::CbcMaximumSeconds,
  ...
  if (!UtilIsInSet(result->m_solStatus2, statusSet2, 4)) {
    cerr << "Warning: CBC IP solver 2nd status = " << result->m_solStatus2
         << endl;
    // This shouldn't really cause an exception
...
void DecompAlgoPC::solveMasterAsMIPCpx(DecompSolverResult *result) {
  ...
  if (logIpLevel) {
    ...
    if (status)
      throw UtilException("CPXsetintparam failure", "solveMasterAsMIP",
                          "DecompSubModel");
  ...
  if (status)
    throw UtilException("CPXsetdblparam failure", "solveMasterAsMIP",
                        "DecompAlgoPC");
  ...
  const int statusSet[5] = {CPXMIP_OPTIMAL, CPXMIP_OPTIMAL_TOL,
                            CPXMIP_INFEASIBLE, CPXMIP_TIME_LIM_FEAS,
  ...
  if (!UtilIsInSet(result->m_solStatus, statusSet, 5)) {
    ...
    throw UtilException("CPX solver status", "solveMasterAsMIP",
                        "DecompAlgoPC");
  }
  ...
  if (status)
    throw UtilException("CPXgetbestobjval failure", "solveMasterAsMIP",
                        "DecompAlgoPC");
  ...
  if (result->m_nSolutions >= 1) {
    ...
    if (status)
      throw UtilException("CPXgetmipobjval failure", "solveMasterAsMIP",
                          "DecompAlgoPC");
...
void DecompAlgoPC::addCutsToPool(const double *x, DecompCutList &newCuts,
                                 int &n_newCuts) {
  ...
  while (li != newCuts.end()) {
    ...
    for (r = 0; r < modelCore->getNumRows(); r++) {
      ...
      if (modelCore->rowHash[r] == (*li)->getStrHash()) {
        ...
        if (relViol > 0.005) { // 0.5% violated
          (*m_osLog) << "CUT actViol= " << actViol << " relViol= " << relViol
                     << "\n";
          (*li)->print(m_osLog);
    ...
    if (isDupCore) {
      ...
      for (ci = m_cutpool.begin(); ci != m_cutpool.end(); ci++) {
        if ((*li)->getStrHash() == (*ci).getCutPtr()->getStrHash()) {
          UTIL_MSG(m_param.LogLevel, 3,
                   (*m_osLog)
                       << "CUT " << cutIndex << " is Duplicate with Pool Cut "
                       << cutIndexPool << endl;
                   (*m_osLog)
                   << "CUT           Hash = " << (*li)->getStrHash() << endl;
    ...
    if (addCut) {
      ...
      CoinPackedVector *rowReform =
          m_cutpool.createRowReform(modelCore->getNumCols(), row, m_vars);
      int tempIndex(0);
...
int DecompAlgoPC::addCutsFromPool() {
  ...
  for (index = 0; index < n_newrows; index++) {
    ...
    case 'L': {
      ...
      colName = "sCL(c_" + UtilIntToStr(colIndex) + "_" +
                UtilIntToStr(rowIndex) + ")";
    ...
    case 'G': {
      ...
      m_masterArtCols.push_back(colIndex);
      colName = "sCG(c_" + UtilIntToStr(colIndex) + "_" +
                UtilIntToStr(rowIndex) + ")";
      colNames.push_back(colName);
      colIndex++;
    } break;
  ...
  //---
  if (rowNames.size() > 0)
    m_masterSI->setRowNames(rowNames, 0, static_cast<int>(rowNames.size()),
                            rowIndex0);
