...
// =========================================================================
// ------------------------------------------------------------------------- //
CoinPackedVector *UtilPackedVectorFromDense(const int len, const double *dense,
                                            const double etol) {
...
// ------------------------------------------------------------------------- //
void UtilPackedVectorFromDense(const int len, const double *dense,
                               const double etol, CoinPackedVector &v) {
  // TODO: test for dup? - efficiency vs debug
  // TODO: insert is slow - better to use setVector?
  int i;
...
// ------------------------------------------------------------------------- //
void UtilPrintPackedVector(const CoinPackedVector &v, ostream *os,
                           DecompApp *app) {
  (*os).precision(2);
...
void UtilPrintPackedVector(const CoinPackedVector &v, ostream *os,
  ...
  for (int i = 0; i < len; i++) {
    if (!namesSize)
      (*os) << setw(10) << UtilDblToStr(elems[i], 4) << " x[" << setw(6)
            << inds[i] << "]  ";
    else {
      (*os) << setw(10) << UtilDblToStr(elems[i], 4) << " " << setw(10)
            << colNames[inds[i]] << " ";
    }
