...
void blockNumberFinder(DecompParam utilParam, std::vector<int> &blockNums,
                       const CoinPackedMatrix *matrix);

void DecompAuto(DecompApp milp, UtilParameters &utilParam, UtilTimer &timer,
...
int main(int argc, char **argv) {
  try {
    ...
    if (milp.m_param.LogDebugLevel > 1) {
      std::cout << "The number of cores is " << numCPU << std::endl;
    }
    ...
    int numThreads =
        min(min(numCPU, static_cast<int>(blockNumCandidates.size())),
            milp.m_param.ConcurrentThreadsNum);
    std::vector<DecompApp> milpArray(static_cast<int>(numThreads + 1), milp);
    std::vector<DecompMainParam> decompMainParamArray(
        static_cast<int>(numThreads + 1), decompMainParam);
...
#pragma omp parallel for
      ...
      for (int i = 0; i < (numThreads + 1); i++) {
        ...
        DecompAuto(milpArray[i], utilParamArray[i], timerArray[i],
                   decompMainParamArray[i]);
      }
    } else {
    ...
    if (milp.m_param.Concurrent == true) {
      ...
      cout << "Method" << setw(20) << "BlockNumber" << setw(20)
           << "WallClockTime" << setw(20) << "CPUTime" << setw(20) << "BestLB"
           << setw(25) << "BestUB" << endl;
      ...
      for (int i = 0; i < (numThreads + 1); i++) {
        ...
        cout << setw(25) << setprecision(7)
             ...
             << setw(23) << setprecision(7) << decompMainParamArray[i].bestLB
             << setw(25) << setprecision(7) << decompMainParamArray[i].bestUB
             << endl;
      }
    }
  } catch (CoinError &ex) {
 ...
 *     table
 */
void blockNumberFinder(DecompParam decompParam, std::vector<int> &blockNums,
                       const CoinPackedMatrix *matrix) {
  ...
  if (decompParam.LogDebugLevel >= 1) {
    ...
    std::string path1 =
        decompParam.CurrentWorkingDir + UtilDirSlash() + "histogramTable.dat";
  ...
  for (histIter = histogram.begin(); histIter != histogram.end(); ++histIter) {
    int keyvalue_pre = histIter->second;
    int max = 0;
    std::map<int, int>::iterator histIterTemp = histIter;
    ++histIterTemp;

    for (histIter2 = histIterTemp; histIter2 != histogram.end(); ++histIter2) {
      ...
      if (keyvalue_pre == keyvalue_curr) {
        max_inter = (histIter->first > histIter2->first) ? histIter->first
                                                         : histIter2->first;
      }
    ...
    if (histogram2.find(histIter->second) == histogram2.end()) {
      histogram2.insert(std::pair<int, int>(histIter->second, histIter->first));
    }
  }
  ...
  if (decompParam.LogDebugLevel >= 1) {
    ...
    std::string path2 =
        decompParam.CurrentWorkingDir + UtilDirSlash() + "histogramTable1.dat";
  ...
  int blockCands = std::min(decompParam.NumBlocksCand -
                                static_cast<int>(blocksNumTemp.size()),
  ...
  if (decompMainParam.doCut && decompMainParam.doDirect) {
    ...
    cout << "BestLB        = " << setw(10)
         ...
         << UtilDblToStr(UtilCalculateGap(alpsModel.getGlobalLB(),
                                          ...
                                          milp.getDecompAlgo()->getInfinity()),
                         5)
         << endl
         << "Nodes         = " << alpsModel.getNumNodesProcessed() << endl
         << "SetupCPU      = " << decompMainParam.timeSetupCpu << endl
         << "SolveCPU      = " << decompMainParam.timeSolveCpu << endl
         << "TotalCPU      = "
         << decompMainParam.timeSetupCpu + decompMainParam.timeSolveCpu << endl
         << "SetupWallclock= " << decompMainParam.timeSetupReal << endl
         << "SolveWallclock= " << decompMainParam.timeSolveReal << endl
         << "TotalWallclock= "
         << decompMainParam.timeSetupReal + decompMainParam.timeSolveReal
         << endl;
    ...
    */
    ...
    if (milp.m_param.SolutionOutputToFile && alpsModel.getGlobalUB() < 1.e100) {
      ...
      osSolution << " " << alpsModel.getGlobalUB() << std::endl;
      ...
      if (alpsModel.getSolStatus() == AlpsExitStatusOptimal) {
        ...
        cout << " Optimal Solution can be found in the file " << solutionFile
             << endl;
      }
    }
...
DecompSolverResult *solveDirect(const DecompApp &decompApp) {
  ...
  if (decompApp.m_param.DataDir != "") {
    fileName =
        decompApp.m_param.DataDir + UtilDirSlash() + decompApp.m_param.Instance;
  } else {
  ...
  DecompSolverResult *result =
      new DecompSolverResult(m_problemSI->getInfinity());
