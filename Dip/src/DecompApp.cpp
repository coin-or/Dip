...
// --------------------------------------------------------------------- //
int DecompApp::generateCuts(const double *x, DecompCutList &newCuts) {
  // ---
  // --- this function does nothing by default
  // ---
...
/*-------------------------------------------------------------------------*/
void DecompApp::printOriginalColumn(const int index, ostream *os) const {
  (*os) << index << " ";
}
...
void DecompApp::initializeApp() {
  UtilPrintFuncBegin(m_osLog, m_classTag, "initializeApp()", m_param.LogLevel,
                     2);
...
#pragma omp critical
   ...
   */
  createModels();
  UtilPrintFuncEnd(m_osLog, m_classTag, "initializeApp()", m_param.LogLevel, 2);
}
...
void DecompApp::readProblem() {
  ...
  if (!foundFormat) {
    cerr << "Error: Format = " << m_param.InstanceFormat << " unknown." << endl;
...
void DecompApp::readBlockFile() {
  ...
  if (m_param.BlockFileFormat == "List" || m_param.BlockFileFormat == "LIST") {
    //---
    ...
    while (!is.eof()) {
      ...
      if (rowNameToIdIt != rowNameToId.end()) {
        ...
        if (m_param.LogLevel >= 3) {
          (*m_osLog) << "Warning: Row name (" << rowName << " in block file "
    ...
    cerr << "Error: BlockFileFormat = " << m_param.BlockFileFormat
         << " is an invalid type. Valid types = (List,ZIBList,Pair,PairName)."
         << endl;
    throw UtilException("Invalid Parameter.", "readBlockFile", "DecompApp");
  }
...
void DecompApp::readInitSolutionFile(DecompVarList &initVars) {
  ...
  ifstream is;
  string fileName = m_param.DataDir + UtilDirSlash() + m_param.InitSolutionFile;
  ...
  for (it = varTemp.begin(); it != varTemp.end(); it++) {
    ...
    for (i = 0; i < static_cast<int>(columnPair.first.size()); i++) {
      origCost += columnPair.second[i] * m_objective[columnPair.first[i]];
    }

    DecompVar *var =
        new DecompVar(columnPair.first, columnPair.second, -1.0, origCost);
...
void DecompApp::createModelPart(DecompConstraintSet *model, const int nRowsPart,
                                const int *rowsPart) {
  int nCols = 0;
  double *rowLB = NULL;
  double *rowUB = NULL;
...
void DecompApp::createModelPartSparse(DecompConstraintSet *model,
  ...
  for (vit = model->activeColumns.begin(); vit != model->activeColumns.end();
       vit++) {
    origIndex = *vit;
    ...
    model->pushCol(colLB[origIndex], colUB[origIndex], isInteger, origIndex);
...
void DecompApp::createModels() {
  UtilPrintFuncBegin(m_osLog, m_classTag, "createModels()", m_param.LogLevel,
                     2);
  //---
  ...
  UTIL_MSG(m_param.LogLevel, 3, (*m_osLog) << "Core  Rows:";

           for (i = 0; i < nRowsCore; i++)(*m_osLog) << rowsCore[i] << " ";
           (*m_osLog) << "\n";);
  ...
  if (m_param.InstanceFormat == "MPS") {
    memcpy(objective, m_mpsIO.getObjCoefficients(), nCols * sizeof(double));
  } else if (m_param.InstanceFormat == "LP") {
    memcpy(objective, m_lpIO.getObjCoefficients(), nCols * sizeof(double));
  }
  ...
  for (mit = m_blocks.begin(); mit != m_blocks.end(); mit++) {
    ...
    if (m_param.LogLevel >= 1)
      (*m_osLog) << "Create model part nRowsRelax = " << nRowsRelax
                 << " (Block=" << (*mit).first << ")" << endl;
      ...
      if (m_param.LogLevel >= 3) {
        if (modelCore->getColNames().size() > 0)
          (*m_osLog) << "Column " << setw(5) << i << " -> " << setw(25)
                     << modelCore->colNames[i] << " is not in union of blocks."
                     << endl;
      }
  ...
  if (m_param.LogLevel >= 3) {
    ...
    if (modelCore->getColNames().size() > 0)
      UtilPrintVector(modelCore->masterOnlyCols, modelCore->getColNames(),
                      m_osLog);
  }

  if (m_param.LogLevel >= 3) {
    std::cout << "the number of masterOnlyCols is "
              << modelCore->masterOnlyCols.size() << std::endl;
  }
  ...
  for (mdi = m_modelR.begin(); mdi != m_modelR.end(); mdi++) {
    ...
    //---
    setModelRelax((*mdi).second, "relax" + UtilIntToStr((*mdi).first),
                  (*mdi).first);
  ...
  UtilPrintFuncEnd(m_osLog, m_classTag, "createModels()", m_param.LogLevel, 2);
  // exit(1);
}
...
void DecompApp::singlyBorderStructureDetection() {
  ...
  UTIL_MSG(m_param.LogLevel, 2,
           (*m_osLog) << "The number of rows is " << numRows << "\n"
  ...
  */
  ...
  PaToH_Alloc(&args, numVertices, numHyperedges, nconst, vwgts, hewgts, eptr,
              eind);
  int cut = 0;
  PaToH_Part(&args, numVertices, numHyperedges, nconst, 0, vwgts, hewgts, eptr,
             eind, NULL, part, partweights, &cut);
  edgecut[0] = cut;
  int computedCut = PaToH_Compute_Cut(nparts, PATOH_CONPART, numVertices,
                                      numHyperedges, hewgts, eptr, eind, part);
  UTIL_MSG(m_param.LogDebugLevel, 2,
           (*m_osLog) << "The computedCut is " << computedCut << "\n";);
#else
  clock_t begin = clock();
#if defined(COIN_HAS_HMETIS)
  // maximum load imbalance (%)
  int ubfactor = 5;
  HMETIS_PartRecursive(numVertices, numHyperedges, vwgts, eptr, eind, hewgts,
  ...
  UTIL_MSG(m_param.LogLevel, 2,
           (*m_osLog) << "The size of the net set after finding the coupling"
                      << "row is " << static_cast<int>(netSet.size()) << "\n";);
  ...
  /*
   * truePartNum indicates the true partition number after eliminating the
   * partitioned blocks where there is no element in the block
   */
  int truePartNum = 0;
...
void DecompApp::setModelRelax(DecompConstraintSet *model,
                              const std::string modelName, const int blockId) {
  ...
  if (mit != m_modelRelax.end()) {
    std::cerr << "Block " << blockId << " relaxation has already been set. "
              ...
              << std::endl;
    throw UtilException("Multiple relaxation definitions", "setModelRelax",
                        "DecompApp");
  }
...
void DecompApp::setInfinity() {
  if (m_param.DecompLPSolver == "Clp") {
    ...
    throw UtilException("Unknown solver selected", "setDecompInf", "DecompApp");
  }
  return;
}
