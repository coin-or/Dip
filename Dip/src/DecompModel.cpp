...
//===========================================================================//
bool DecompSubModel::isPointFeasible(const double *x, const bool isXSparse,
                                     const int logLevel,
  ...
  for (it = activeColumns.begin(); it != activeColumns.end(); it++) {
    ...
    UTIL_DEBUG(
        ...
        if (!UtilIsZero(xj)) {
          ...
          cout << " LB= " << UtilDblToStr(clb, precision)
               ...
               << " UB= " << UtilDblToStr(cub, precision) << endl;
        });
    actViol = std::max<double>(clb - xj, xj - cub);
    actViol = std::max<double>(actViol, 0.0);

    if (UtilIsZero(xj, feasVarTol) || (xj < 0 && UtilIsZero(clb)) ||
        (xj > 0 && UtilIsZero(cub))) {
      relViol = actViol;
    } else {
    ...
    if (relViol > feasVarTol) {
      ...
      UTIL_DEBUG(logLevel, 4, int precision = 7;
                 cout << "Point violates column " << c;

                 if (hasColNames) cout << " -> " << colNames[c];
                 cout << " LB= " << UtilDblToStr(clb, precision)
                      << " x= " << UtilDblToStr(xj, precision)
                      << " UB= " << UtilDblToStr(cub, precision) << " RelViol= "
                      << UtilDblToStr(relViol, precision) << endl;);
  ...
  for (r = 0; r < model->getNumRows(); r++) {
    ...
    if (UtilIsZero(ax, feasConTol) || (ax < 0 && UtilIsZero(rlb)) ||
        (ax > 0 && UtilIsZero(rub))) {
      relViol = actViol;
    } else {
    ...
    if (relViol > feasConTol) {
      // Notify, but don't mark in feasible unless 10x worse.
      UTIL_DEBUG(logLevel, 4, cout << "Point violates row ";

                 if (hasRowNames) cout << " -> " << rowNames[r];
                 cout << " ax[" << r << "]: " << UtilDblToStr(ax) << " LB: "
                      << UtilDblToStr(rlb) << " UB: " << UtilDblToStr(rub)
                      << " RelViol: " << UtilDblToStr(relViol) << endl;);
...
FUNC_EXIT:
  UTIL_DEBUG(logLevel, 4, cout << "isPointFeasible = " << isFeas << endl;);
  return isFeas;
}

//===========================================================================//
void DecompSubModel::solveAsMIP(DecompSolverResult *result, DecompParam &param,
                                bool doExact, bool doCutoff, bool isRoot,
                                double cutoff, double timeLimit) {
  //---
  ...
  if (param.DecompIPSolver == "SYMPHONY") {
    ...
    throw UtilException("Unknown solver selected.", "solveAsMIP",
                        "DecompSubModel");
  }
}
...
void DecompSubModel::solveAsMIPSym(DecompSolverResult *result,
                                   DecompParam &param, bool doExact,
                                   bool doCutoff, bool isRoot, double cutoff,
                                   double timeLimit) {
  ...
  OsiSymSolverInterface *osiSym = dynamic_cast<OsiSymSolverInterface *>(m_osi);
  ...
  if ((status == PREP_OPTIMAL_SOLUTION_FOUND) ||
    ...
    if (param.LogDebugLevel >= 4) {
      std::cout << "The optimal objective value is " << objective_value
                << std::endl;
    }
...
void DecompSubModel::solveAsMIPCbc(DecompSolverResult *result,
  ...
  if (!UtilIsInSet(result->m_solStatus, statusSet, 2)) {
    ...
    throw UtilException("CBC solver status", "solveAsMIPCbc", "DecompSubModel");
  }
#else
  // int i;
  ...
  if (doExact) {
    cbcTimeSet = UtilDblToStr(min(param.SubProbTimeLimitExact, param.TimeLimit),
                              -1, COIN_DBL_MAX);
    cbcGapSet = UtilDblToStr(param.SubProbGapLimitExact, -1, COIN_DBL_MAX);
  } else {
    cbcTimeSet = UtilDblToStr(
        min(param.SubProbTimeLimitInexact, param.TimeLimit), -1, COIN_DBL_MAX);
  ...
  if (!doCutoff && isRoot) {
    if (!UtilIsInSet(result->m_solStatus2, statusSet2a, nSeta)) {
      cerr << "Error: CBC IP solver 2nd status = " << result->m_solStatus2
           << endl;
      throw UtilException("CBC solver 2nd status", "solveAsMIPCbc",
                          "DecompSubModel");
    }
  } else {
  ...
  if (cbc.isContinuousUnbounded()) {
    OsiClpSolverInterface *m_relax =
        dynamic_cast<OsiClpSolverInterface *>(m_osi);
...
void DecompSubModel::solveAsMIPCpx(DecompSolverResult *result,
  ...
  //---
  OsiCpxSolverInterface *osiCpx = dynamic_cast<OsiCpxSolverInterface *>(m_osi);
  ...
  if (logIpLevel) {
    ...
    if (status)
      throw UtilException("CPXsetintparam failure", "solveAsMIPCpx",
                          "DecompSubModel");
  ...
  if (doExact)
    status =
        CPXsetdblparam(cpxEnv, CPX_PARAM_EPGAP, param.SubProbGapLimitExact);
  else
    status =
        CPXsetdblparam(cpxEnv, CPX_PARAM_EPGAP, param.SubProbGapLimitInexact);

  if (status)
    throw UtilException("CPXsetdblparam failure", "solveAsMIPCpx",
                        "DecompSubModel");

  if (doExact) {
    if (param.SubProbTimeLimitExact < COIN_DBL_MAX) {
      status =
          CPXsetdblparam(cpxEnv, CPX_PARAM_TILIM, param.SubProbTimeLimitExact);
    }
  } else {
  ...
  if (nMipStarts > 10) {
    ...
    if (status)
      throw UtilException("CPXdelmipstarts failure", "solveAsMIPCpx",
                          "DecompSubModel");
  }
  ...
  const int statusSet1[6] = {CPXMIP_OPTIMAL,
                             ...
                             CPXMIP_TIME_LIM_FEAS, CPX_STAT_OPTIMAL,
                             CPX_STAT_UNBOUNDED,   CPXMIP_UNBOUNDED};
  const int statusSet2[9] = {
      CPXMIP_OPTIMAL,
      CPXMIP_OPTIMAL_TOL, // for stopping on gap
      CPXMIP_TIME_LIM_FEAS, CPXMIP_INFEASIBLE,  CPXMIP_INForUNBD,
      CPX_STAT_UNBOUNDED,   CPX_STAT_INForUNBD, CPX_STAT_OPTIMAL,
  ...
  if (result->m_solStatus == CPXMIP_INForUNBD ||
    ...
    std::cout << "There might be extreme rays in the subproblems " << std::endl;
    /*
    ...
    */
    ...
    if (status) {
      throw UtilException("XPXsetintparam failure", "solveAsMIPCpx",
                          "DecompSubModel");
    }
    ...
    if (status) {
      throw UtilException("CPXgetray failure", "solveAsMIPCpx",
                          "DecompSubModel");
    }
    ...
    */
    ...
    if (!UtilIsInSet(result->m_solStatus, statusSet2, 9)) {
      ...
      throw UtilException("CPX solver status", "solveAsMIPCpx",
                          "DecompSubModel");
    }
  }
  ...
  if (!doCutoff && isRoot) {
    if (!UtilIsInSet(result->m_solStatus, statusSet1, 6)) {
      cerr << "Error: CPX IP solver 2nd status = " << result->m_solStatus
           << endl;
      throw UtilException("CPX solver status", "solveAsMIPCpx",
                          "DecompSubModel");
    }
  ...
  if (UtilIsInSet(result->m_solStatus, statusSet1, 4)) {
    ...
    for (i = 0; i < nSols; i++) {
      ...
      if (status)
        throw UtilException("CPXgetsolnpoolobjval", "solveAsMIPCpx",
                            "DecompSubModel");

      // printf("Sol %4d: Obj: %10g\n", i, objVal);
      status = CPXgetsolnpoolx(cpxEnv, cpxLp, i, solution, 0, numCols - 1);
      vector<double> solVec(solution, solution + numCols);
      result->m_solution.push_back(solVec);
      result->m_nSolutions++;
  ...
  if (result->m_solStatus == CPXMIP_OPTIMAL ||
    ...
    if (status)
      throw UtilException("CPXgetbestobjval failure", "solveAsMIPCpx",
                          "DecompSubModel");
    ...
    if (result->m_nSolutions >= 1 && !result->m_isUnbounded) {
      ...
      if (status)
        throw UtilException("CPXgetmipobjval failure", "solveAsMIPCpx",
                            "DecompSubModel");
    }
  }
  UTIL_DELARR(solution);
...
void DecompSubModel::solveAsMIPGrb(DecompSolverResult *result,
  ...
  OsiGrbSolverInterface *osiGrb = dynamic_cast<OsiGrbSolverInterface *>(m_osi);
  ...
  if (stat == GRB_OPTIMAL) {
    ...
    throw UtilException("Solution failure", "solveAsMIPGrb", "DecompSubModel");
  }
#else
