...
void DecompConstraintSet::prepareModel(double infinity, bool modelIsCore) {
  ...
  if (isSparse()) {
    ...
    for (mcit = m_sparseToOrig.begin(); mcit != m_sparseToOrig.end(); mcit++) {
    ...
    for (sit = activeColumnsSet.begin(); sit != activeColumnsSet.end(); sit++) {
...
void DecompConstraintSet::createRowHash(double infinity) {
  ...
  for (r = 0; r < getNumRows(); r++) {
    strHash = UtilCreateStringHash(rmat_len[r], rmat_ind + rmat_beg[r],
                                   rmat_els + rmat_beg[r], rowSense[r],
                                   rowRhs[r], infinity);
...
void DecompConstraintSet::boundsToSenses(double infinity) {
  ...
  for (int r = 0; r < n_rows; r++) {
    UtilBoundToSense(rowLB[r], rowUB[r], infinity, sense, rhs, range);
...
CoinPackedMatrix *DecompConstraintSet::sparseToOrigMatrix() {
  ...
  CoinPackedMatrix *MOrig =
      new CoinPackedMatrix(false, nCols, nRows, nElems, matVal, matIndOrig,
                           matBeg, matLen, 0.0, 0.0);
