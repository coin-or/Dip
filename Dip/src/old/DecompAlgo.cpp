...
void DecompAlgo::initSetup(int whichModel)
{
   ...
   //printf("\nm_whichCoreModel: %d -> n_rows: %d\n",
   //       m_whichCoreModel, m_app->m_modelCore[m_whichCoreModel]->getNumRows());
   //printf("\nm_whichModel    : %d -> n_rows: %d\n",
   //       m_whichModel, m_app->m_modelCore[m_whichModel]->getNumRows());
#ifdef CUT_FIRST
   ...
   printf("\nmodelCore  cols: %d, rows: %d",
          m_modelCore->getNumCols(),
          m_modelCore->getNumRows());

   if (m_modelRelax->M) {
      printf("\nmodelRelax cols: %d, rows: %d",
             m_modelRelax->getNumCols(),
             m_modelRelax->getNumRows());
   }
...
// --------------------------------------------------------------------- //
void DecompAlgo::solve(int whichModel)
{
   //actually, nicer if you have a set of cores and relaxes and you
   //whichCoreModel, whichRelaxModel - then can mix and match, esp
   //for this tighten idea...
   //DecompVarList initVars;
   decompPhase   phase              = PHASE_INIT;
   decompStat    stat               = STAT_UNKNOWN;
   int           n_cutCalls         = 0; //number of cut calls in a row
   int           n_priceCalls       = 0; //number of price calls in a row
   int           n_cutCallsTotal    = 0; //number of cut calls in a row
   int           n_priceCallsTotal  = 0; //number of price calls in a row
   ...
   m_modelRelax = m_app->m_modelRelax[m_whichModel];
   UTIL_DEBUG(m_app->m_param.LogDebugLevel, 3,
              (*m_osLog) << m_classTag << " <---- solve() ---- ";
              (*m_osLog) << "\nWHICH MODEL = " << m_whichModel;
             );
   //THINK:
   //for DC, we don't need the m_modelCore!, only m_modelRelax->.. and
   //if called from within DecompAlgoC, we dont' need that at all,
   ...
   if (m_algo != RELAX_AND_CUT)
      UTIL_DEBUG(m_app->m_param.LogDebugLevel, 3,
                 m_masterSI->writeMps("initmps");
                );

   //exit(1);//for SAS
   //if(m_algo == PRICE_AND_CUT)
   //  exit(1);
   //?? check for no-cols cut in DC
   ...
   //n_cutCallsTotal, n_priceCallsTotal,
   //                      m_cutsThisCall,  n_newVars,
   //                      n_cutCalls, n_priceCalls);
   else {
      phase = PHASE_PRICE;
   }

   //TODO - when do i search for IP feasibility - see cut
   //this whole part should be for procesing a node... if we are going to
   //be branching...
   ...
   while (phase != PHASE_DONE) {
      ...
      if (phase != PHASE_DONE) {
         ...
         if ((m_cutsThisCall + m_varsThisCall) > 0) {
            if (m_algo != RELAX_AND_CUT)
               UTIL_DEBUG(m_app->m_param.LogDebugLevel, 50,
                          string masterMpsFile = "masterProb."
                                                 + UtilIntToStr(n_cutCallsTotal) + "."
                                                 + UtilIntToStr(n_priceCallsTotal);
                          m_masterSI->writeMps(masterMpsFile.c_str());
                          m_masterSI->writeLp(masterMpsFile.c_str(),
         ...
         //
         //---
         //--- is this the right place to run user heuristics?
         //---
         //TODO
      }
   } //while(phase != PHASE_DONE)
                              ...
                              const double   intTol)
{
   ...
   UTIL_DEBUG(m_app->m_param.LogDebugLevel, 0,
