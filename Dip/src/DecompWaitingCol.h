...
public:
   ...
   inline DecompRay*         getRayPtr() const {
      return m_ray;
   }
   inline CoinPackedVector* getColVarPtr() const {
      return m_col_var;
   }
   inline CoinPackedVector* getColRayPtr() const {
      return m_col_ray;
   }

   inline const double getVarReducedCost() const  {
      return m_var->getReducedCost();
   }
   inline const double getRayReducedCost() const  {
      return m_ray->getReducedCost();
   }

   inline const double getVarLowerBound() const   {
      return m_var->getLowerBound();
   }
   inline const double getVarUpperBound() const   {
      return m_var->getUpperBound();
   }

   inline const double getRayLowerBound() const   {
      return m_ray->getLowerBound();
   }
   inline const double getRayUpperBound() const   {
      return m_ray->getUpperBound();
   }

   inline const double getVarOrigCost() const     {
      return m_var->getOriginalCost();
   }
   inline const double getRayOrigCost() const     {
      return m_ray->getOriginalCost();
   }

   inline void   deleteColVar() {
      UTIL_DELPTR(m_col_var);
   }
   inline void   deleteColRay() {
      UTIL_DELPTR(m_col_ray);
   }
   ...
   inline void   clearRay()  {
      m_ray = 0;
   }

   inline void   setColVar(CoinPackedVector* col) {
      m_col_var = col;
   }
   inline void   setColRay(CoinPackedVector* col) {
      m_col_ray = col;
   }
