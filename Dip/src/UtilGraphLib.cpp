...
void UtilGraphLib::read_data(const char *datafile) {
  ...
  static char keywords[KEY_NUM][22] = {"NAME",
                                       "NAME:",
                                       "TYPE",
                                       "TYPE:",
                                       "COMMENT",
  ...
  static char wtypes[WTYPE_NUM][9] = {"EXPLICIT", "EUC_2D", "EUC_3D", "MAX_2D",
  ...
  */
  ...
  while (0 != fgets(line1, LENGTH, f)) {
    ...
    if (k == KEY_NUM) {
      cerr << "UtilGraphLib::read_data ERROR I/O : unknown keyword " << key
           << ". Aborting." << endl;
      abort();
    }
    ...
    case 0: { // NAME (set name)
      if (!sscanf(line, "%s", tmp)) {
        cerr << "UtilGraphLib::read_data ERROR I/O : reading NAME" << tmp
             << ". Aborting." << endl;
        abort();
      }
    ...
    case 3: // DIMENSION (set n_vertices, n_edges)
      ...
      if (!sscanf(line, "%d", &k)) {
        cerr << "UtilGraphLib::read_data ERROR I/O : reading DIMENSION " << k
             << ". Aborting." << endl;
        abort();
      }
    ...
    case 4: // CAPACITY (set capacity)
      ...
      if (!sscanf(line, "%d", &k)) {
        cerr << "GrabLib:: read_data ERROR I/O : reading CAPACITY " << k
             << ". Aborting." << endl;
        abort();
      }
    ...
    case 5: // EDGE_WEIGHT_TYPE
      ...
      if (wtype == WTYPE_NUM) {
        cerr << "GrabLib::read_data ERROR I/O : unknown weight type " << tmp
             << ". Aborting." << endl;
        abort();
      }
